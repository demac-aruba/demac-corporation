...
function endpoint(functionName) {
  ...
  url.pathname = url.pathname.replace(/wacliWebhook\/?$/, functionName);
  return url.toString();
}
...
async function wacli(args) { const { stdout } = await execFileAsync(WACLI_BINARY, ['--json', '--read-only', ...args], { timeout: 30000, maxBuffer: 16 * 1024 * 1024, env: process.env, windowsHide: true }); const parsed = JSON.parse(String(stdout || '').trim() || '{}'); if (parsed?.success === false) throw new Error(parsed?.error?.message || 'wacli failed'); return parsed?.data ?? parsed; }
...
async function postBackfill(payload) { const raw = Buffer.from(JSON.stringify(payload)); const response = await fetch(endpoint('wacliBackfillUpdate'), { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Wacli-Signature': signature(raw) }, body: raw }); const data = await response.json().catch(() => ({})); if (!response.ok || !data.ok) throw new Error(data.error || `backfill ${response.status}`); return data; }

await fs.mkdir(TEMP_DIR, { recursive: true, mode: 0o700 });
const chatList = await wacli(['chats', 'list', '--limit', '2000']).catch(() => []);
const chatRows = rows(chatList);
const messageList = await wacli(['messages', 'list', '--limit', String(LIMIT)]);
...
for (const row of messageRows) {
  ...
  let detail; try { detail = await wacli(['messages', 'show', '--chat', base.chat, '--id', base.id]); } catch { continue; }
  ...
  let avatar = null;
  if (!avatarDone.has(base.chat) && !base.chat.endsWith('@g.us')) {
    avatarDone.add(base.chat);
    try {
      const info = await wacli(['profile', 'picture-info', '--jid', base.chat]); const imageUrl = strings(info).find((item) => /url/.test(item.key) && /^https:\/\//.test(item.value))?.value;
      if (imageUrl) { const response = await fetch(imageUrl); if (response.ok) { const buffer = Buffer.from(await response.arrayBuffer()); const contentType = response.headers.get('content-type') || 'image/jpeg'; const storagePath = await upload(buffer, { scope: 'avatar', identity: base.chat, conversationId: base.chat, messageId: `avatar-${Date.now()}`, contentType, fileName: 'profile.jpg', kind: 'image' }); avatar = { storagePath, updatedAt: new Date().toISOString() }; } }
