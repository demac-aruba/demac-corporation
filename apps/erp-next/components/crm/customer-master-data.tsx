...
export type { PropertyEditorValue } from '@/lib/property-editor-draft';
  ...
  return (
    <section className={styles.masterPanel}>
      ...
      {tab === 'Properties' ? <div className={styles.recordList}>
        {graph.properties.map((property) => <article className={styles.siteCard} key={property.id}><div className={styles.siteIcon}><PropertyIcon name={property.hasIndependentDwellings ? 'building' : 'house'} /></div><div className={styles.recordMain}><div><strong>{propertyName(property)}</strong><b>{property.active === false ? 'Inactive' : 'Active'}</b></div><span>{property.address || property.addressRaw || 'Address pending'}</span><small>{property.neighborhood || property.zone || property.operationalZone || 'Area pending'} · {property.type || 'Property'}{property.dwellingCount ? ` · ${property.dwellingCount} unidades` : ''}</small>{property.accessInstructions ? <em>{property.accessInstructions}</em> : null}</div><button type="button" onClick={() => setEditor({ kind: 'property', mode: 'edit', requestId: createOfficeLifecycleRequestId('crm-property-update'), initial: propertyEditorValue(property) })}>Edit property</button></article>)}
      ...
      {editor?.kind === 'property' ? <PropertyEditor customerId={graph.client.id} customerName={graph.client.company || graph.client.name || graph.client.id} contacts={graph.contacts} mode={editor.mode} requestId={editor.requestId} initial={editor.initial} onClose={() => setEditor(null)} onSave={editor.mode === 'edit' ? onUpdateProperty : onAddProperty} /> : null}
    </section>
  );
}
