...
function tone(status: AttendanceStatus | null) {
...
export function EmployeeAttendanceWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!selectedEmployeeId) setSelectedEmployeeId(canonical.staffProfiles.find((profile) => profile.active !== false)?.id ?? canonical.staffProfiles[0]?.id ?? '');
    } catch (cause) {
  ...
  function employeeVan(profile: CanonicalStaffProfile | null) {
  ...
  function scheduleForDate(profile: CanonicalStaffProfile, date: string): AttendanceSchedule {
  ...
  function selectDate(value: string) {
    ...
    const record = recordsFor(selectedEmployee, value);
  ...
  const monthMetrics = useMemo(() => {
    if (!selectedEmployee) return { present: 0, late: 0, sick: 0, vacation: 0, absent: 0 };
  ...
  async function save() {
    ...
    try {
      ...
      setAttendance((current) => ({
        ...current,
        timesheets: [saved, ...current.timesheets.filter((entry) => entry.id !== saved.id)],
      }));
  ...
  const linkedVan = employeeVan(selectedEmployee);

  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <section className="panel">
        <header className="panel-head">
          <div><h2>Attendance Calendar</h2><span>Operational absences come from staffAbsences; attendance and payroll exceptions continue using the Legacy employeeTimesheets records.</span></div>
          <button className="btn" type="button" onClick={() => void load()} disabled={loading}>{loading ? 'Refreshing…' : 'Refresh'}</button>
        </header>
        ...
        <div style={styles.metrics}>
          <Metric label="Present" value={monthMetrics.present} />
          <Metric label="Late" value={monthMetrics.late} />
          <Metric label="Sick / AO" value={monthMetrics.sick} />
          <Metric label="Vacation" value={monthMetrics.vacation} />
          <Metric label="Absent / NWNP" value={monthMetrics.absent} />
        </div>
      </section>
      ...
      <section style={styles.layout}>
        <article className="panel" style={{ overflow: 'hidden' }}>
          ...
          <div style={styles.calendar}>
            {monthCells.map((date, index) => {
              ...
              return (
                <button key={date} type="button" onClick={() => selectDate(date)} style={{ ...styles.day, ...(selected ? styles.selectedDay : {}) }}>
                  <span style={{ fontWeight: 900 }}>{Number(date.slice(-2))}</span>
                  <small>{tone(record.status)}</small>
                  {record.entry?.lateMinutes ? <small>{record.entry.lateMinutes}m late</small> : null}
                  {record.entry?.workedMinutes ? <small>{hoursAndMinutes(record.entry.workedMinutes)} worked</small> : null}
                </button>
              );
            })}
          </div>
        ...
        <aside className="panel" style={{ padding: 18, alignSelf: 'start' }}>
          <div className="eyebrow">Daily Record</div>
          <h2 style={{ marginTop: 4 }}>{formatDate(selectedDate)}</h2>
          <p style={{ opacity: 0.72 }}>{selectedRecord.schedule.label}</p>
          {selectedRecord.absence && !selectedRecord.absence.id.startsWith('attendance-') ? <div style={styles.warning}><strong>Existing absence range</strong><div>{selectedRecord.absence.reason} · {selectedRecord.absence.fromDate} → {selectedRecord.absence.toDate}</div><small>This record is preserved; the attendance editor will not silently delete a pre-existing range absence.</small></div> : null}
          {draft ? (
            <div style={{ display: 'grid', gap: 13, marginTop: 16 }}>
              ...
              <div style={styles.twoCol}>
                <label style={styles.field}><strong>Clock in</strong><input type="time" value={draft.clockInTime} onChange={(event) => setDraft({ ...draft, clockInTime: event.target.value })} /></label>
                <label style={styles.field}><strong>Clock out</strong><input type="time" value={draft.clockOutTime} onChange={(event) => setDraft({ ...draft, clockOutTime: event.target.value })} /></label>
              </div>
              <div style={styles.twoCol}>
                <label style={styles.field}><strong>Break minutes</strong><input type="number" min="0" step="5" value={draft.breakMinutes} onChange={(event) => setDraft({ ...draft, breakMinutes: Number(event.target.value) })} /></label>
                <label style={styles.field}><strong>Overtime minutes</strong><input type="number" min="0" step="5" value={draft.overtimeMinutes} onChange={(event) => setDraft({ ...draft, overtimeMinutes: Number(event.target.value) })} /></label>
              </div>
              <label style={styles.field}><strong>Notes</strong><textarea rows={4} value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} placeholder="Late arrival reason, sick notice, vacation reference, payroll note…" /></label>
              ...
              <button className="btn primary" type="button" onClick={() => void save()} disabled={busy || !canManageSensitiveAttendance}>{busy ? 'Saving…' : 'Save Daily Record'}</button>
              {!canManageSensitiveAttendance ? <small>Read-only: sensitive attendance/payroll editing is restricted to the owner/administrator or authorized finance.</small> : null}
            </div>
          ) : null}
        </aside>
      </section>
