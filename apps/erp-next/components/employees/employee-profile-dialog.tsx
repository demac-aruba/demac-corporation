...
export function EmployeeProfileDialog({
  ...
  const profile = draft ? (operations.staffProfiles.find((item) => item.id === draft.id) as LifecycleProfile | undefined) : undefined;
  const linkedUser = draft ? userByStaffId.get(draft.id) : undefined;

  useEffect(() => {
    if (!open || !draft || !employee || !canManageAccess || !linkedUser) return;
    setDraft((current) => current ? draftFromProfile(employee as LifecycleProfile, linkedUser) : current);
  ...
  const selectedHalfDay = selectedVanId
    ? operations.vanHalfDaySchedules.find((rule) => canonicalVanId(rule.vanId, operations.vans) === selectedVanId)
    : undefined;
  const selectedAbsences = profile
    ? operations.staffAbsences.filter((absence) => absence.staffId === profile.id && absence.active !== false)
      .sort((a, b) => String(b.fromDate ?? '').localeCompare(String(a.fromDate ?? ''))).slice(0, 5)
    : [];
      ...
      <section className={styles.modal} role="dialog" aria-modal="true" aria-label={isNew ? 'Add employee' : 'Employee profile'}>
        ...
        <div className={styles.modalBody}>
          {!isNew ? <div className={styles.summaryGrid}>
            ...
            <Summary label="Employment" value={profile?.employmentStartedAt ? `Since ${profile.employmentStartedAt}` : 'Start date not recorded'} />
          </div> : null}

          <Section title="Personal & Employment Information" subtitle="This is the canonical staff profile used by scheduling, attendance, payroll references and historical work records.">
          ...
          <Section title="ERP Access & Login" subtitle="Operational role and system permission remain separate. Contact email never silently becomes a login credential.">
            {canManageAccess ? <>
              <div className={styles.formGrid}>
          ...
          {!isNew ? <Section title="Work Schedule & Time Off" subtitle="Schedule references remain in their canonical scheduling sources; time off writes to staffAbsences so Scheduling and Attendance share the same truth.">
            ...
            {profile?.active !== false && canManageEmployees ? <div className={styles.timeOffBox}>
              ...
              <div className={styles.inlineActions}><button className={styles.button} type="button" onClick={() => void addTimeOff()} disabled={busy}>Save Time Off</button></div>
            </div> : null}
          ...
          {!isNew && profile ? <Section title="Employment Lifecycle" subtitle="Former employees are archived, never deleted, so historical work, attendance and payroll references remain intact.">
            {profile.active !== false ? <div className={styles.dangerZone}><div><strong>Offboard employee</strong><span>Disables access and removes future crew assignments while preserving history.</span></div><button className={styles.dangerButton} type="button" onClick={() => setOffboardDraft({ endDate: today, reason: '', releaseLoginEmail: Boolean(linkedUser) && (profile.loginEmailKind ?? defaultLoginEmailKind(inferredEmployeeType(profile))) === 'company' })} disabled={!canManageAccess || busy}>Start Offboarding</button></div>
              : <div className={styles.dangerZone}><div><strong>Former employee</strong><span>{profile.employmentEndedAt ? `Employment ended ${profile.employmentEndedAt}. ` : ''}{profile.offboardingReason ?? ''}</span></div><button className={styles.button} type="button" onClick={() => void confirmReactivate()} disabled={!canManageAccess || busy}>Reactivate Employee</button></div>}
          </Section> : null}

          {message ? <div className={styles.success}>{message}</div> : null}
          {error ? <div className={styles.error}>{error}</div> : null}
          {(isNew || profile?.active !== false) && canManageEmployees ? <div className={styles.footer}><button className={styles.button} type="button" onClick={onClose} disabled={busy}>Cancel</button><button className={styles.primaryButton} type="button" onClick={() => void saveEmployee()} disabled={busy}>{busy ? 'Saving…' : 'Save Employee'}</button></div> : null}
        </div>
      </section>

      {offboardDraft ? <div className={styles.backdrop} style={{ zIndex: 1500 }} role="presentation">
        <section className={styles.modal} style={{ width: 'min(620px,100%)' }} role="dialog" aria-modal="true" aria-label="Offboard employee">
          <header className={styles.modalHeader}><div><span className={styles.eyebrow}>Employment lifecycle</span><h2>Offboard {profile ? staffDisplayName(profile) : draft.name}</h2><p>Archive safely; never delete historical employee records.</p></div><button className={styles.button} type="button" onClick={() => setOffboardDraft(null)} disabled={busy}>Close</button></header>
          <div className={styles.modalBody}>
            <div className={styles.formGrid}>
              <Field label="Last employment date"><input className={styles.control} type="date" value={offboardDraft.endDate} onChange={(event) => setOffboardDraft({ ...offboardDraft, endDate: event.target.value })} /></Field>
              <Field label="Reason"><input className={styles.control} value={offboardDraft.reason} onChange={(event) => setOffboardDraft({ ...offboardDraft, reason: event.target.value })} placeholder="Resigned, contract ended, terminated…" /></Field>
            </div>
            <label className={styles.checkCard}><input type="checkbox" checked={offboardDraft.releaseLoginEmail} onChange={(event) => setOffboardDraft({ ...offboardDraft, releaseLoginEmail: event.target.checked })} /><span><strong>Release reusable company login email</strong><small>The former identity is archived; the email may then be assigned to a replacement employee.</small></span></label>
            <div className={styles.warning}>Regular van membership and future dated assignments are removed. Historical work orders, visits, attendance, payroll and audit records remain linked to this employee identity.</div>
            {error ? <div className={styles.error}>{error}</div> : null}
            <div className={styles.footer}><button className={styles.button} type="button" onClick={() => setOffboardDraft(null)} disabled={busy}>Cancel</button><button className={styles.dangerButton} type="button" onClick={() => void confirmOffboard()} disabled={busy}>{busy ? 'Offboarding…' : 'Confirm Offboarding'}</button></div>
          </div>
        </section>
      </div> : null}
    </div>
...
function Section({ title, subtitle, children }: { title: string; subtitle: string; children: ReactNode }) {
  return <section className={styles.section}><header className={styles.sectionHeader}><h3>{title}</h3><p>{subtitle}</p></header>{children}</section>;
}
...
function newEmployeeDraft(): EmployeeDraft {
  return { id: `staff-${crypto.randomUUID()}`, name: '', phone: '', contactEmail: '', employeeType: 'Secretaria', role: 'Secretaria', canDriveVan: false, skillsText: '', notes: '', employmentStartedAt: todayKey(), createAccess: true, accessRole: 'office', accessActive: true, loginEmail: '', loginEmailKind: 'company' };
}
