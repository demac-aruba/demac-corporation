...
import { emptyDraft, validateStep, type ApplicationDraft } from '../../lib/careers-preview';
...
export function CareersPublic() {
  ...
  function normalize(route: CareerRoute): CareerRoute {
    ...
    if (route.view === 'detail' || route.view === 'form') {
      ...
      for (let i = 0; i < step; i++) if (Object.keys(validateStep(draft, job, i)).length) { step = i; review = false; break; }
      if (review && Object.keys(validateStep(draft, job, 2)).length) review = false;
  ...
  async function submit(): Promise<string | null> {
    ...
    try {
      ...
      return e instanceof Error ? e.message : 'Unable to submit. Please retry.';
    }
  ...
  async function reviewUpdatedPosition() {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to refresh the position. Your answers are still here.'); }
