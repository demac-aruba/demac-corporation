    ...
    <section className={styles.queue}>
      ...
      <div className={styles.layout}>
        ...
        {selected ? <main className={styles.detail}>
          ...
          <div className={styles.reviewControls}><label>Customer report language<select value={selected.language} onChange={(event) => updateSelected({ language: event.target.value as BrowserOfficeReviewRecord['language'] })}><option>English</option><option>Spanish</option><option>Papiamento</option></select></label><label className={styles.note}>Office review note<textarea rows={3} value={selected.reviewerNote ?? ''} onChange={(event) => updateSelected({ reviewerNote: event.target.value })} placeholder="Corrections, customer wording, quality note..." /></label></div>
          ...
          <footer><button type="button" className={styles.returnButton} onClick={() => decide('returned')}>Return for Correction</button><button type="button" className={styles.approveButton} onClick={() => decide('approved')}>Approve Report</button></footer>
        </main> : null}
      </div>
    </section>
  );
