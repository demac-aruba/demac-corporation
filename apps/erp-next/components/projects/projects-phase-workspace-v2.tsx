...
function nextProjectNumber(projects: BrowserProject[]) {
  const numbers = projects
    .map((project) => /^PRJ-(\d+)$/.exec(project.projectNumber.trim().toLocaleUpperCase('en')))
    .map((match) => Number(match?.[1] ?? 0))
    .filter((value) => Number.isInteger(value) && value > 0);
  return `PRJ-${Math.max(1007, ...numbers) + 1}`;
}
...
function CanonicalCreateProjectDialog({
  existingProjects,
  busy,
  onClose,
  onSave,
}: {
  existingProjects: BrowserProject[];
  busy: boolean;
  onClose: () => void;
  onSave: (result: ProjectCreateResult) => Promise<void>;
}) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try {
      if (!references) throw new Error('Canonical CRM is not available. Retry customer data before saving.');
      if (!capacity) throw new Error('Estimated work days must be a positive whole number.');
      const name = String(form.get('name') ?? '').trim();
      const customerNameInput = customerQuery.trim();
      if (!name || !customerNameInput) throw new Error('Project name and customer are required.');

      let customer = references.clients.find((candidate) => candidate.id === resolvedCustomer?.id && candidate.active !== false);
      let customerCreated = false;
      if (!customer) {
        if (!createCustomer) throw new Error('Select an existing CRM customer or explicitly choose Create new customer.');
        ...
        const duplicate = references.clients.find((candidate) => {
          const phones = [candidate.phone, candidate.whatsapp]
            .map((value) => normalizeBookingPhone(text(value)))
            .filter(Boolean);
          return phones.includes(phone)
            || Boolean(email && text(candidate.email).toLocaleLowerCase('en') === email.toLocaleLowerCase('en'));
        });
        if (duplicate) throw new Error(`This phone, WhatsApp, or email already belongs to ${customerLabel(duplicate)}. Select that existing customer instead.`);
      ...
      const projectNumber = String(form.get('projectNumber') ?? '').trim().toLocaleUpperCase('en');
      if (!projectNumber) throw new Error('Project number is required.');
      if (existingProjects.some((candidate) => candidate.projectNumber.trim().toLocaleUpperCase('en') === projectNumber)) {
        throw new Error(`Project number ${projectNumber} already exists.`);
      }
      const propertyId = String(form.get('propertyId') ?? '').trim();
      const property = references.properties.find((candidate) => candidate.id === propertyId && candidate.clientId === customer.id && candidate.active !== false);
      if (propertyId && !property) throw new Error('Select an active Service Property that belongs to the selected customer.');
      const startsOn = String(form.get('startsOn') ?? '').trim();
      const estimatedCompletionOn = String(form.get('endsOn') ?? '').trim();
      if (!startsOn || !estimatedCompletionOn || estimatedCompletionOn < startsOn) throw new Error('Enter a valid project date range.');
      const totalUnits = Number(form.get('units'));
      ...
      const materialBudget = projectTypeUsesMaterialBudget(projectType) ? materialBudgetValue : null;
      ...
      const project: BrowserProject = {
        ...
        type: projectType,
        description: String(form.get('description') ?? '').trim() || `${name} · ${projectType}.`,
      ...
      await onSave({ project, customerCreated, customerName });
    } catch (cause) {
      setSaveError(cause instanceof Error ? cause.message : 'The project could not be saved.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return <div className={styles.overlay} role="presentation" onMouseDown={(event) => {
    ...
    <section className={styles.modal} role="dialog" aria-modal="true" aria-label="Create Project">
      <header><div><span>Projects · Canonical CRM</span><h2>Create Project</h2><p>Search the same customer and property records used by CRM and Scheduling. A new CRM customer is created only after you explicitly select that option and save.</p></div><button type="button" onClick={onClose} disabled={saving || busy}>×</button></header>
      <form onSubmit={submit}>
        <div className={styles.formGrid}>
          <label><span>Project number *</span><input name="projectNumber" required defaultValue={nextProjectNumber(existingProjects)} /></label>
          ...
          <label className={styles.formWide}><span>Project name *</span><input name="name" required autoFocus placeholder="Enter the project name" /></label>

          <div className={`${styles.formWide} ${customerStyles.customerPicker}`}>
            ...
            />
            <small id="project-customer-help" className={customerStyles.fieldHelp}>
              {loading ? 'Searching canonical CRM customers and Service Properties…'
                : loadError ? 'Customer search is unavailable; retry before saving.'
                  : ambiguousCustomer ? 'More than one active CRM customer has this exact name. Select the correct record below.'
                    : resolvedCustomer ? `Existing CRM customer selected · ${resolvedCustomer.id}`
                      : createCustomer ? `New canonical CRM customer: ${customerQuery.trim()}`
                        : 'Select an existing customer or explicitly create a new one.'}
            </small>

            {loadError ? <div className={customerStyles.inlineError} role="alert"><span>{loadError}</span><button type="button" onClick={() => void refreshReferences()}>Retry CRM</button></div> : null}
            {!selectedCustomerId && !createCustomer && customerQuery.trim() && !loading && !loadError ? <div id="project-customer-results" className={customerStyles.customerResults} role="listbox" aria-label="Customer search results">
              {results.map((candidate) => {
                const property = propertiesForCustomer(references, candidate.id)[0];
                return <button key={candidate.id} type="button" role="option" aria-selected={resolvedCustomer?.id === candidate.id} onClick={() => chooseCustomer(candidate)}>
                  <span><strong>{customerLabel(candidate)}</strong><small>{text(candidate.company) && text(candidate.company) !== customerLabel(candidate) ? candidate.company : text(candidate.phone) || text(candidate.email) || candidate.id}</small></span>
                  <em>{text(property?.address) || text(candidate.zone) || 'No property yet'}</em>
                </button>;
              })}
              {canOfferCreate ? <button type="button" className={customerStyles.createCustomerOption} onClick={() => { setSelectedCustomerId(''); setCreateCustomer(true); }}>
          ...
          {resolvedCustomer && customerProperties.length ? <label className={styles.formWide}><span>Property / service location</span><select name="propertyId" defaultValue={customerProperties.length === 1 ? customerProperties[0].id : ''} key={resolvedCustomer.id}><option value="">Select later — project remains Draft</option>{customerProperties.map((property) => <option key={property.id} value={property.id}>{text(property.name) || 'Property'} · {text(property.address) || text(property.zone) || property.id}</option>)}</select></label> : null}
          {resolvedCustomer && !customerProperties.length ? <div className={`${styles.formWide} ${customerStyles.inlineNote}`}>This customer has no active Service Property yet. The project can remain Draft, but Scheduling will require a property before work is booked.</div> : null}
          {createCustomer ? <div className={`${styles.formWide} ${customerStyles.inlineNote}`}>The new customer will be created in canonical CRM when you save. No property, appointment, Work Order, or Scheduling capacity will be created automatically.</div> : null}

          <label><span>Total units *</span><input name="units" type="number" min="1" step="1" required defaultValue="1" /></label>
...
export function ProjectsPhaseWorkspaceV2() {
  ...
  const createProject = async ({ project: created, customerCreated, customerName }: ProjectCreateResult) => {
    ...
    try {
      ...
      showNotice(customerCreated
        ? `${customerName} was created in canonical CRM and ${created.projectNumber} was created for phase planning.`
        : `${created.projectNumber} was linked to ${customerName}. You can now create its phases.`);
    } finally {
      setBusy(false);
    }
  };
  ...
  if (view === 'portfolio' || !project || !summary || !metrics) {
    return <section className={styles.workspace} aria-busy={busy}>
      ...
      {createProjectOpen ? <CanonicalCreateProjectDialog existingProjects={state.projects} busy={busy} onClose={() => setCreateProjectOpen(false)} onSave={createProject} /> : null}
    </section>;
  }
