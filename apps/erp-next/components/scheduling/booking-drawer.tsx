...
export type BookingPrefill = { workLines?: BookingWorkLine[]; restriction?: BookingRestriction; sector?: string; source?: 'copilot' };
...
function workLinesFromPrefill(prefill?: BookingPrefill): WorkLineDraft[] {
  if (!prefill?.workLines?.length) return [workLine(0)];
  return prefill.workLines.map((line, index) => ({ id: line.id || `work-${Date.now()}-${index}`, presetId: line.presetId, quantityInput: String(line.quantity) }));
}
...
function restrictionDescription(value?: BookingRestriction) {
  if (!value) return 'No customer time restriction';
  return describeBookingConstraints({ halfDay: value.halfDay, notBefore: value.notBefore, notAfter: value.notAfter });
}
...
export function BookingDrawer({ day, jobs, preferred, prefill, onClose, onReserve, onApplySupportReflow }: Props) {
  const directSpotRequested = Boolean(preferred.vanId && preferred.start);
  ...
  const [workLines, setWorkLines] = useState<WorkLineDraft[]>(() => workLinesFromPrefill(prefill));
  ...
  const parsedWorkLines = useMemo<BookingWorkLine[]>(() => workLines.map((line) => ({ id: line.id, presetId: line.presetId, quantity: Number(line.quantityInput) })), [workLines]);
  ...
  const effectiveRestriction = directSpotRequested ? prefill?.restriction : bookingRestrictionFromConstraints(restrictionState(restriction));
  ...
  }), [customer, effectiveRestriction, primaryLine.presetId, safeWorkLines, sector, site, totalQuantity]);
  ...
  }, [customerId, day, jobs, preferred.start, preferred.vanId, request, scopeValid, sector, siteId]);

  const exactPreferred = useMemo(() => directSpotRequested ? slots.find((slot) => slot.vanId === preferred.vanId && slot.start === preferred.start) : undefined, [directSpotRequested, preferred.start, preferred.vanId, slots]);
  ...
  const liveIssues = useMemo(() => {
    if (!customerId || !siteId || !sector) return [];
    return diagnoseBookingRequest({ request, jobs, preferred, candidateSlots: slots, quantityValid: scopeValid });
  }, [customerId, jobs, preferred, request, scopeValid, sector, siteId, slots]);
  ...
  const clearSelection = () => { setSelected(null); setSelectedOffer(null); setOfferNotice(null); };
  ...
  const chooseCustomer = (id: string, preferredSiteId?: string) => {
    ...
    if (!crmCustomer) { setCustomer(''); setSite(''); setCrmSites([]); setSector(''); return; }
    ...
    if (firstSite) {
      ...
      setSector(sectorFromCrm(crmCustomer, firstSite) ?? '');
    } else {
      setSite('');
      setSector('');
    }
  ...
  const chooseSite = (id: string) => {
    ...
    if (!crmSite) { setSite(''); setSector(''); return; }
    setSite(crmSite.name);
    setSector(sectorFromCrm(selectedCustomer, crmSite) ?? '');
  };
  ...
  const chooseCreatedCustomer = (result: QuickCustomerCreateResult) => {
    ...
    setSector(sectorFromCrm(result.customer, primary) ?? '');
    clearSelection();
  };
  ...
  const addPropertyToExisting = (result: QuickExistingPropertyResult) => {
    ...
    setSector(sectorFromCrm(customerRecord, result.site) ?? result.site.sector ?? '');
    clearSelection();
  };

  const updateWorkLine = (id: string, patch: Partial<WorkLineDraft>) => { setWorkLines((current) => current.map((line) => line.id === id ? { ...line, ...patch } : line)); clearSelection(); };
  const chooseSlot = (slot: CandidateSlot) => { setSelected(slot); setSelectedOffer(createBookingOffer({ dayKey: day.dateKey, request, slot, jobs })); setOfferNotice(null); };
  ...
  const optionsContent = !customerId || !siteId ? <div className={styles.noSlots}><strong>Customer and property required</strong><p>Choose the CRM relationship and exact service property before the ERP validates capacity.</p></div>
    : !sector ? <div className={styles.noSlots}><strong>Property sector needs confirmation</strong><p>This property does not yet have a reliable DEMAC sector. Update the property location before booking instead of guessing a sector here.</p></div>
      : !scopeValid ? <div className={styles.noSlots}><strong>Valid work scope required</strong><p>Every work line needs a whole-number quantity from 1 to 14.</p></div>
        : directSpotRequested && exactPreferred ? <div className={styles.descriptionPreview}><span>SELECTED WORK SPOT · BOOKING INTELLIGENCE VALIDATED</span><strong>{exactPreferred.vanId.replace('VAN-', 'Van ')} · {formatTime(exactPreferred.start)}–{formatTime(exactPreferred.end)}</strong><small>{exactPreferred.sector} · {exactPreferred.reasons.join(' · ') || 'Valid continuous capacity'}{exactPreferred.supportVanId ? ` · Support ${exactPreferred.supportVanId.replace('VAN-', 'Van ')}` : ''}</small></div>
          : slots.length ? <>{directSpotRequested ? <div className={styles.noSlots} style={{ borderLeft: '3px solid var(--warning)' }}><strong>The selected work spot no longer fits</strong><p>Booking Intelligence kept your customer and work scope, but the clicked spot is not valid anymore. Choose one of the alternatives below.</p></div> : null}{slots.map((slot, index) => <button type="button" key={`${slot.vanId}-${slot.start}-${slot.supportVanId ?? ''}-${slot.supportStart ?? ''}`} className={`${styles.slotOption} ${selected === slot ? styles.slotOptionSelected : ''}`} onClick={() => chooseSlot(slot)}><div><strong>{index === 0 ? 'BEST ROUTE · ' : ''}{slot.vanId.replace('VAN-', 'Van ')} · {formatTime(slot.start)}–{formatTime(slot.end)}</strong><span>{slot.sector}{slot.supportVanId ? ` · support ${slot.supportVanId.replace('VAN-', 'Van ')}${slot.supportStart ? ` ${formatTime(slot.supportStart)}–${formatTime(slot.supportEnd ?? slot.end)}` : ''}` : ''}</span></div><b>{slot.score}</b><small>{slot.reasons.join(' · ') || 'Valid capacity'}</small></button>)}</>
            : reflowPlans.length ? <><div className={styles.noSlots} style={{ borderLeft: '3px solid var(--brand)', background: 'var(--brand-soft)' }}><strong>Booking Intelligence found recoverable capacity</strong><p>A support-only assignment can move without changing its primary customer appointment.</p></div>{reflowPlans.map((plan) => <button type="button" key={plan.id} className={styles.slotOption} onClick={() => { clearSelection(); onApplySupportReflow(plan); }}><div><strong>Recover {plan.vanId.replace('VAN-', 'Van ')} · move support {formatTime(plan.fromStart)} → {formatTime(plan.toStart)}</strong><span>{plan.customer} · {plan.quantity} support unit{plan.quantity === 1 ? '' : 's'} · {plan.sector}</span></div><b>APPLY</b><small>Unlocks {formatTime(plan.unlockedSlot.start)}–{formatTime(plan.unlockedSlot.end)} · Primary appointment remains unchanged</small></button>)}</>
              : <div className={styles.noSlots}><strong>No valid capacity for this request</strong><p>ERP checked duration, route, lunch, return buffer, customer restrictions, linked support and safe support reflow.</p></div>;
  ...
  return <div className={styles.drawerOverlay} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className={styles.drawer} role="dialog" aria-modal="true">
      <header className={styles.drawerHeader}><div><span>New appointment · {day.weekday} {day.shortDate}</span><h2>{directSpotRequested ? 'Complete booking for selected work spot' : 'Find the best valid work spot'}</h2><p>{directSpotRequested ? `${preferred.vanId?.replace('VAN-', 'Van ')} · ${preferred.start ? formatTime(preferred.start) : ''}. The ERP validates the spot in the background and only shows alternatives if it stops fitting.` : 'CRM identity, exact property, work scope, route, customer restrictions and van capacity are evaluated by one deterministic booking core.'}</p></div><button type="button" onClick={onClose}>×</button></header>
      <div className={styles.drawerBody}>
        ...
        <section className={styles.formSection}><header><strong>1 · Customer & property</strong><span>Property address and DEMAC sector are authoritative CRM data.</span></header>
          <div className={styles.formGrid}>
            <label className={styles.wide}><span>CRM customer search</span><input value={customerQuery} onChange={(event) => { setCustomerQuery(event.target.value); if (customerId) { setCustomerId(''); setCustomer(''); setSiteId(''); setSite(''); setSector(''); setCrmSites([]); clearSelection(); } }} placeholder="Name, phone / WhatsApp, or email..." /></label>
            <div className={styles.wide} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}><span style={{ color: 'var(--muted)' }}>{selectedCustomer ? `${selectedCustomer.id} · ${selectedCustomer.name}` : hasCustomerQuery ? `${customerMatches.length} CRM match${customerMatches.length === 1 ? '' : 'es'}` : 'Start typing to search CRM'}</span><button type="button" className={styles.secondary} onClick={() => setCustomerCreateOpen(true)}>+ Add Customer / Property</button></div>
          </div>
          {!selectedCustomer ? <div className={styles.slotOptions}>{!hasCustomerQuery ? <div className={styles.noSlots}><strong>Search only when needed</strong><p>No random customers are shown. Start typing a name, phone/WhatsApp or email.</p></div> : customerMatches.length ? customerMatches.map((item) => <button type="button" key={item.id} className={styles.slotOption} onClick={() => chooseCustomer(item.id)}><div><strong>{item.name}</strong><span>{item.location || item.type || 'Customer'} · {item.phone || 'No phone'}</span></div><b>Select</b><small>{item.email || item.id}</small></button>) : <div className={styles.noSlots}><strong>No customer found</strong><p>Create the CRM relationship instead of booking an unregistered duplicate.</p><button type="button" className={styles.secondary} onClick={() => setCustomerCreateOpen(true)}>+ Add Customer / Property</button></div>}</div> : <div className={styles.descriptionPreview}><span>SELECTED CRM CUSTOMER</span><strong>{selectedCustomer.name}</strong><small>{selectedCustomer.phone || 'No phone'} · {selectedCustomer.email || 'No email'}</small></div>}
          {customerId ? <div className={styles.formGrid}><label className={styles.wide}><span>Registered property</span><select value={siteId} onChange={(event) => chooseSite(event.target.value)}><option value="">Select a property</option>{crmSites.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.address}</option>)}</select></label><label><span>DEMAC sector · from property</span><input value={sector || 'Needs property location confirmation'} readOnly /></label>{!crmSites.length ? <div className={`${styles.wide} ${styles.noSlots}`}><strong>No registered property</strong><p>Add the new property here without leaving the booking flow.</p><button type="button" className={styles.secondary} onClick={() => setCustomerCreateOpen(true)}>+ Add Property to Existing Customer</button></div> : null}</div> : null}
          {propertyIssues.map((issue) => <LiveIssue issue={issue} key={issue.code} />)}
        </section>

        <section className={styles.formSection}><header><strong>2 · Appointment scope{directSpotRequested ? '' : ' & restrictions'}</strong><span>{directSpotRequested ? 'The clicked work spot already defines the scheduling period.' : 'One visit can contain multiple work lines; ERP calculates combined continuous duration.'}</span></header>
          ...
          <div style={{ margin: '9px 11px', display: 'flex', gap: 8, flexWrap: 'wrap' }}><button type="button" className={styles.secondary} onClick={() => { setWorkLines((current) => [...current, workLine(current.length)]); clearSelection(); }}>+ Add another work type</button>{!directSpotRequested ? <label style={{ minWidth: 180 }}><span>Customer restriction</span><select value={restriction} onChange={(event) => { setRestriction(event.target.value); clearSelection(); }}><option value="any">No time restriction</option><option value="morning">Morning only</option><option value="afternoon">Afternoon only</option><option value="after10">After 10:00 AM</option><option value="after2">After 2:00 PM</option></select></label> : prefill?.restriction ? <div className={styles.descriptionPreview} style={{ flex: 1, margin: 0 }}><span>INHERITED CUSTOMER TIME CONSTRAINT</span><strong>{restrictionDescription(prefill.restriction)}</strong></div> : null}</div>
        ...
        <section className={styles.formSection}><header><strong>3 · {directSpotRequested ? 'Selected work spot' : 'Best valid ERP options'}</strong><span>{directSpotRequested ? `Requested: ${preferred.vanId?.replace('VAN-', 'Van ')} · ${preferred.start ? formatTime(preferred.start) : ''}` : 'Hard constraints first; valid choices are ranked for route efficiency.'}</span></header>
          {slotIssues.map((issue) => <LiveIssue issue={issue} key={issue.code} />)}
          <div className={styles.slotOptions}>{optionsContent}</div>
        </section>
      </div>
      <footer className={styles.drawerFooter}><div>{selected ? <><span>Selected · revalidated on commit</span><strong>{selected.vanId.replace('VAN-', 'Van ')} · {formatTime(selected.start)}{selected.supportVanId ? ` + ${selected.supportVanId.replace('VAN-', 'Van ')} support` : ''}</strong></> : <span>{directSpotRequested ? 'Complete customer/property data to validate the selected spot.' : 'Select a valid work spot.'}</span>}</div><div><button type="button" className={styles.secondary} onClick={onClose}>Cancel</button><button type="button" className={styles.primary} disabled={!selected || !selectedOffer || !customerId || !siteId || !sector || !scopeValid} onClick={reserveSelected}>Temporary hold</button></div></footer>
    </aside>
