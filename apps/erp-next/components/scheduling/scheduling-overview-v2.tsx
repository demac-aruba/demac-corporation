...
import { BookingCopilot } from './booking-copilot';
import { BookingDrawer, type BookingIdentity, type BookingPrefill, type PreferredSlot } from './booking-drawer';
...
export function SchedulingOverviewV2() {
  ...
  const openBooking = (slot?: PreferredSlot, prefill?: BookingPrefill) => {
  ...
  const useCopilotPlan = (plan: BookingCopilotPlan, prefill: Omit<BookingPrefill, 'source'>) => {
    if (!canManage) return;
    if (plan.kind === 'capacity_recovery' && plan.recoveryPlan) applySupportReflow(plan.recoveryPlan);
    setActiveDate(plan.dateKey);
    setPreferredSlot({ vanId: plan.slot.vanId, start: plan.slot.start });
    setBookingPrefill({ ...prefill, source: 'copilot' });
    setCopilotOpen(false);
    setSelectedAppointmentId(null);
    setPendingDragMove(null);
    disarmMove();
    setDrawerOpen(true);
    setNotice(plan.kind === 'capacity_recovery'
      ? 'Capacity recovery confirmed. Complete the customer/property details; the recovered work spot will be revalidated before the hold is saved.'
      : 'Simulation selected. Complete the customer/property details; the exact work spot will be revalidated before the hold is saved.');
  };
  ...
  return <section className={styles.page}>
    <header className={styles.pageHeader}><div><span className={styles.eyebrow}>Operations · Aruba</span><h1>Scheduling & Dispatch</h1><p>See the operational week, all four vans and every available work spot from one primary agenda view.</p></div><div className={styles.pageActions}><button type="button" className={styles.secondary} disabled={!canManage} onClick={() => { setDrawerOpen(false); setSelectedAppointmentId(null); setCopilotOpen(true); }}>🎙 Booking Copilot</button><button type="button" className={styles.secondary}>Capacity settings</button><button type="button" className={styles.primary} disabled={!activeDay.isOpen || !canManage} onClick={() => openBooking()}>+ New appointment</button></div></header>
    ...
    <div className={styles.toolbar}><div className={styles.legend}><span><i className={styles.readyDot} /> Ready</span><span><i className={styles.riskDot} /> At risk</span><span><i className={styles.blockedDot} /> Blocked</span><span><i className={styles.holdDot} /> Hold</span><span><i className={styles.openDot} /> Open spot</span></div></div>
