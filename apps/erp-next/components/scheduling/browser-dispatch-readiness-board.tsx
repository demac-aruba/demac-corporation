...
export function BrowserDispatchReadinessBoard() {
  ...
  const readyCount = primaryRows.filter((row) => row.readiness.status === 'ready').length;
  const riskHoldCount = primaryRows.filter((row) => row.readiness.status === 'at_risk' && row.startDecision.mode === 'at_risk_hold').length;
  const riskReleasedCount = primaryRows.filter((row) => row.readiness.status === 'at_risk' && row.startDecision.mode === 'released_at_risk').length;
  const blockedCount = primaryRows.filter((row) => row.readiness.status === 'blocked').length;

  return (
    <section className={styles.board}>
      <header>
        <div><span>LIVE PRE-DISPATCH CONTROL</span><h2>Dispatch Readiness Board</h2><p>Scheduling owns date, time and van assignment. This board projects the current eight-dimension readiness decision and start authority without changing the appointment or Work Order.</p></div>
      ...
      <div className={styles.metrics}>
        <article><span>READY</span><strong className={styles.good}>{readyCount}</strong><small>Can start normally</small></article>
        <article><span>AT RISK · HOLD</span><strong className={riskHoldCount ? styles.warn : ''}>{riskHoldCount}</strong><small>No valid Operations release</small></article>
        <article><span>AT RISK · RELEASED</span><strong className={riskReleasedCount ? styles.warn : ''}>{riskReleasedCount}</strong><small>Start allowed under exact release</small></article>
        <article><span>BLOCKED</span><strong className={blockedCount ? styles.danger : ''}>{blockedCount}</strong><small>No Field override</small></article>
        ...
        {vanIds.map((vanId) => {
          ...
          return <section className={styles.lane} key={vanId}>
            ...
            <div className={styles.cards}>{laneRows.length ? laneRows.map((row) => {
              ...
              const started = Boolean(row.execution?.startedAt);
              const statusClass = row.readiness.status === 'ready' ? styles.cardReady : row.readiness.status === 'blocked' ? styles.cardBlocked : row.startDecision.mode === 'released_at_risk' ? styles.cardReleased : styles.cardRisk;
              return <article className={`${styles.jobCard} ${statusClass}`} key={`${row.order.id}-${row.vanId}-${row.role}`}>
                ...
                <div className={styles.readiness}><div><span>READINESS</span><strong>{labelStatus(row.readiness.status)}</strong></div><p>{row.readiness.status === 'blocked' ? blockerReasons || row.startDecision.reason : row.readiness.status === 'at_risk' ? riskReasons || row.startDecision.reason : 'All eight readiness dimensions are resolved.'}</p></div>
                {row.readiness.status === 'at_risk' ? <div className={styles.authority}><span>{row.startDecision.mode === 'released_at_risk' ? 'OPERATIONS RELEASE ACTIVE' : 'START AUTHORITY ON HOLD'}</span><strong>{row.startDecision.release ? row.startDecision.release.reason : row.startDecision.reason}</strong>{row.startDecision.release ? <small>{row.startDecision.release.id} · {row.startDecision.release.authorizedBy}</small> : null}</div> : null}
                {started ? <div className={styles.started}><span>FIELD STARTED</span><strong>{row.execution?.startedAt ? new Date(row.execution.startedAt).toLocaleString() : 'Started'}</strong><small>{row.execution?.startAuthority === 'released_at_risk' ? `Under release ${row.execution.dispatchReleaseId ?? 'unknown'}` : row.execution?.startAuthority === 'ready' ? 'Started from READY authority' : 'Legacy start authority'}</small></div> : null}
      ...
      <footer><span>READ-ONLY PROJECTION</span><strong>Changing readiness happens in the owning source module. Changing date/time/van happens in Scheduling. This board only combines the evidence for dispatch decisions.</strong></footer>
    </section>
  );
}
