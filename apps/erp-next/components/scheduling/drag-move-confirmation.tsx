...
export function DragMoveConfirmation({ move, onCancel, onConfirm }: { move: PendingDragMove; onCancel: () => void; onConfirm: () => void }) {
  ...
  >
    <aside role="dialog" aria-modal="true" aria-labelledby="drag-confirm-title" style={{ width: 'min(440px, calc(100vw - 32px))', overflow: 'hidden', border: '1px solid var(--border)', borderRadius: 14, background: 'var(--surface)', boxShadow: '0 24px 70px rgba(10, 24, 44, .28)' }}>
      ...
      <footer style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '12px 20px 16px', borderTop: '1px solid var(--border)' }}>
        <button type="button" className={styles.secondary} onClick={onCancel}>No, keep original</button>
        <button type="button" className={styles.primary} onClick={onConfirm}>Yes, confirm move</button>
      </footer>
    </aside>
  </div>;
}
