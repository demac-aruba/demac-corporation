...
function ProgressPanel({ progress }: { progress?: CreativeProgress }) {
  ...
  const steps = [
    ['intelligence', 'Mode + brief'],
    ['explore', '12 territories'],
    ['diversity', 'Diversity + blueprints'],
  ...
  return <section className={styles.panel} style={{ border: '1px solid #b7d5ff', background: '#f8fbff' }}>
    <header><span>CREATIVE ENGINE V4</span><h2>{progress?.label || 'Starting Performance Design Intelligence pipeline…'}</h2></header>
    ...
    <p style={{ margin: '12px 0 0', color: '#68778d', fontSize: 12 }}>V4 diagnoses the ad type before exploring layouts. Do not refresh while the page is polling live progress from Firebase.</p>
  </section>;
}
...
export function CreativeStudioV2({ initialView = 'studio' }: { initialView?: View }) {
  ...
  const build = async () => {
    ...
    try {
      ...
      await refresh();
      setNotice('Creative Engine V4 completed. Review the campaign mode, brief, blueprints, visual benchmark, performance benchmark, and selected winner below.');
    } catch (buildError) {
  ...
  return <div className={styles.workspace}>
    <section className={styles.hero}>
      <div><div className={styles.eyebrow}>DEMAC MARKETING AGENT · PERFORMANCE DESIGN INTELLIGENCE V4</div><h1>{view === 'approved' ? 'Approved Creatives' : 'Creative Studio V4'}</h1><p>{view === 'approved' ? 'Agency-benchmark creatives approved for export and publishing.' : 'creative mode → structured brief → 12 territories → diversity blueprints → full AI designs → visual + performance QA → refine top 2 → executive jury.'}</p></div>
    ...
    <section className={styles.metrics}>
      ...
      <article><span>Creative Builds</span><strong>{creatives.length}</strong><small>V1 → V4 preserved</small></article>
      <article><span>QA Passed</span><strong>{creatives.filter((creative) => creative.qa?.status === 'passed').length}</strong><small>Agency + performance benchmark</small></article>
...
function StudioView({ data, sessions, selectedSessionId, setSelectedSessionId, selectedSession, selectedCampaign, selectedHeroUrl, selectedCreatives, latestCreative, working, onBuild, onApprove, onRegenerateStrategy }: {
  ...
  return <div className={styles.stack}>
    <section className={styles.controlPanel}>
      <div><span>CAMPAIGN INPUT</span><h2>Select campaign</h2><p>V4 first decides what kind of ad the evidence supports, then explores twelve territories and spends image-generation budget only on four materially different blueprints.</p></div>
      ...
      <button type="button" className={styles.primaryButton} disabled={!ready || Boolean(working)} onClick={onBuild}>{working === 'build' ? `${selectedSession?.creativeProgress?.percent || 5}% · Building V4…` : latestCreative ? 'Run New V4 Exploration' : 'Build V4 Creative Campaign'}</button>
    </section>
...
function CreativeReview({ creative, working, onApprove }: { creative: CreativeRecord; working: string; onApprove: (creative: CreativeRecord) => void }) {
  ...
  const v4 = creative.builderVersion === 'V4';
  const advanced = v3 || v4;
  const design = creative.designIntelligence;
  const brief = creative.creativeBrief;
  return <div className={styles.stack}>
    {v4 && brief ? <section className={styles.panel}><header><span>V4 CAMPAIGN INTELLIGENCE</span><h2>{(brief.creativeMode || creative.creativeMode || 'creative').replaceAll('_', ' ')}</h2></header><p style={{ marginTop: 0, color: '#66758b' }}>{brief.creativeNorthStar}</p><div className={styles.strategyGrid}><div><span>Conversion goal</span><p>{brief.conversionGoal || '—'}</p></div><div><span>Primary promise</span><p>{brief.primaryPromise || '—'}</p></div><div><span>Hero asset role</span><p>{brief.heroAssetRole || '—'}</p></div><div><span>Mode confidence</span><strong>{brief.modeConfidence || 0}/100</strong></div></div>{brief.supportingProof?.length ? <div style={{ marginTop: 12 }}><strong style={{ fontSize: 12 }}>SUPPORTING PROOF</strong><ul>{brief.supportingProof.map((item) => <li key={item} style={{ margin: '6px 0', fontSize: 12, color: '#66758b' }}>{item}</li>)}</ul></div> : null}</section> : null}

    {advanced && design ? <section className={styles.panel}><header><span>{v4 ? 'V4 DESIGN INTELLIGENCE' : 'DESIGN INTELLIGENCE'}</span><h2>{design.explorationCount || 12} explored → {design.shortlistCount || 4} finalists → {design.refinementCount || 2} refined</h2></header><p style={{ marginTop: 0, color: '#66758b' }}>{design.creativeNorthStar || creative.artDirection?.creativeNorthStar}</p><div className={styles.strategyGrid}><div><span>Benchmark</span><p>{design.benchmarkDefinition || 'Professional paid-social agency standard.'}</p></div><div><span>Portfolio / Jury</span><p>{v4 ? design.portfolioRationale || design.finalJury?.reason : design.finalJury?.reason || 'Executive jury result unavailable.'}</p></div><div><span>Spend confidence</span><strong>{design.finalJury?.spendConfidence || 0}/100</strong></div><div><span>Provider</span><strong>{creative.providerManifest?.activeProvider || 'openai_full_design'}</strong></div></div></section> : null}

    {variants.length ? <section className={styles.panel}><header><span>{advanced ? 'FINALISTS + REFINEMENTS' : 'CREATIVE CONCEPTS'}</span><h2>{variants.length} preserved rendered candidates · AI winner selected</h2></header><p style={{ marginTop: 0, color: '#66758b' }}>{creative.artDirection?.creativeNorthStar || 'Materially distinct concepts generated from the same real DEMAC photo.'}</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: 16 }}>{variants.map((variant) => {
      const selected = variant.id === creative.selectedVariantId;
      return <article key={variant.id} style={{ border: selected ? '2px solid #1769e0' : '1px solid #dce3ec', borderRadius: 16, padding: 10, background: selected ? '#f6faff' : '#fff' }}><div style={{ position: 'relative' }}><img src={variant.imageUrl} alt={variant.name} style={{ width: '100%', aspectRatio: '1/1', objectFit: 'cover', borderRadius: 12 }} />{selected ? <b style={{ position: 'absolute', top: 10, left: 10, padding: '6px 9px', borderRadius: 999, background: '#1769e0', color: '#fff', fontSize: 10 }}>AI WINNER</b> : null}</div><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', marginTop: 10 }}><strong>{variant.name}</strong><span className={statusClass(variant.qa.status)}>{variant.qa.status.toUpperCase()}</span></div><p style={{ fontSize: 12, color: '#66758b', minHeight: 48 }}>{variant.rationale}</p><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', fontSize: 11 }}><span>{variant.stage || (variant.revised ? 'refined' : 'candidate')}</span><span>Selection {variant.selectionScore || 0}</span><span>Visual {variant.qa.visualBenchmarkLevel || variant.qa.benchmarkLevel || 'legacy'}</span>{variant.qa.performanceBenchmarkLevel ? <span>Performance {variant.qa.performanceBenchmarkLevel}</span> : null}{variant.qa.adSpendReady ? <span>Spend-ready</span> : null}</div></article>;
    })}</div></section> : null}

    <section className={styles.reviewGrid}>
      <article className={styles.creativePanel}><header><div><span>SELECTED CREATIVE</span><h2>Version {creative.version} · {creative.builderVersion || 'V1'}</h2></div><b className={statusClass(creative.status)}>{statusLabel(creative.status)}</b></header><div className={styles.imageFrame}><img src={creative.approvedUrl || creative.imageUrl} alt={`DEMAC creative version ${creative.version}`} /></div><div className={styles.renderMeta}><span>{creative.width}×{creative.height}</span><span>{advanced ? '12→4→2 Design Intelligence' : creative.renderTemplate.replaceAll('_', ' ')}</span><span>{creative.imageModel || creative.renderMode}</span><span>Footer Reserve {creative.reservedFooterPx}px</span></div>{qaPassed && !approved ? <button type="button" className={styles.primaryButton} disabled={Boolean(working)} onClick={() => onApprove(creative)}>{working === 'approve' ? 'Approving…' : 'Approve Creative'}</button> : null}{approved ? <a href={creative.approvedUrl || creative.imageUrl} target="_blank" rel="noreferrer" className={styles.primaryButton}>Open Approved PNG</a> : null}<div className={styles.copyPreview}><span>SOCIAL CAPTION</span><p>{creative.captionText}</p></div></article>

      <article className={styles.panel}><header><span>{v4 ? 'VISUAL + PERFORMANCE BENCHMARK QA V4' : v3 ? 'PAID-MEDIA BENCHMARK QA V3' : 'STRICT VISUAL QA'}</span><h2>{creative.qa?.score || 0}/100 <b className={statusClass(creative.qa?.status || 'needs_review')}>{(creative.qa?.status || 'review').toUpperCase()}</b></h2></header>{v4 ? <><div style={{ marginBottom: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}><span className={styles.warnPill}>Visual: {(creative.qa?.visualBenchmarkLevel || 'unknown').replaceAll('_', ' ')}</span><span className={styles.warnPill}>Performance: {(creative.qa?.performanceBenchmarkLevel || 'unknown').replaceAll('_', ' ')}</span><span className={creative.qa?.adSpendReady ? styles.goodPill : styles.badPill}>{creative.qa?.adSpendReady ? 'AD SPEND READY' : 'NOT SPEND READY'}</span>{creative.qa?.visibleTextExact ? <span className={styles.goodPill}>TEXT EXACT</span> : <span className={styles.badPill}>TEXT CHECK</span>}{creative.qa?.hardFailure ? <span className={styles.badPill}>HARD FAIL</span> : null}</div><strong style={{ fontSize: 12 }}>VISUAL DESIGN</strong>{metric('Composition', creative.qa?.visualReview?.composition)}{metric('Typography', creative.qa?.visualReview?.typography)}{metric('Professional Finish', creative.qa?.visualReview?.professionalFinish)}{metric('Brand Coherence', creative.qa?.visualReview?.brandCoherence)}{metric('Photo Authenticity', creative.qa?.visualReview?.photoAuthenticity)}{metric('Originality', creative.qa?.visualReview?.originality)}{metric('Mobile Readability', creative.qa?.visualReview?.mobileReadability)}<strong style={{ fontSize: 12, display: 'block', marginTop: 16 }}>PERFORMANCE MARKETING</strong>{metric('Scroll-stopping', creative.qa?.performanceReview?.scrollStopping)}{metric('Promise Clarity', creative.qa?.performanceReview?.promiseClarity)}{metric('Proof Strength', creative.qa?.performanceReview?.proofStrength)}{metric('CTA Prominence', creative.qa?.performanceReview?.ctaProminence)}{metric('Conversion Path', creative.qa?.performanceReview?.conversionPath)}{metric('Audience Relevance', creative.qa?.performanceReview?.audienceRelevance)}{metric('Creative Mode Fit', creative.qa?.performanceReview?.modeFit)}{creative.qa?.hardFailureReasons?.length ? <div><strong style={{ fontSize: 12 }}>HARD FAILURE REASONS</strong><ul>{creative.qa.hardFailureReasons.map((issue) => <li key={issue} style={{ margin: '7px 0', fontSize: 12, color: '#9a3f3f' }}>{issue}</li>)}</ul></div> : null}{creative.qa?.amateurSignals?.length ? <div><strong style={{ fontSize: 12 }}>AMATEUR SIGNALS</strong><ul>{creative.qa.amateurSignals.map((issue) => <li key={issue} style={{ margin: '7px 0', fontSize: 12, color: '#9a3f3f' }}>{issue}</li>)}</ul></div> : null}</> : v3 ? <>{metric('Creative Direction', creative.qa?.creativeDirection)}{metric('Composition', creative.qa?.composition)}{metric('Typography', creative.qa?.typography)}{metric('Professional Finish', creative.qa?.professionalFinish)}{metric('Brand Distinctiveness', creative.qa?.brandDistinctiveness)}{metric('Conversion Clarity', creative.qa?.conversionClarity)}{metric('Thumbnail Impact', creative.qa?.thumbnailImpact)}{metric('Authenticity', creative.qa?.authenticity)}{metric('Text Fidelity', creative.qa?.textFidelity)}{metric('Footer Safety', creative.qa?.footerSafety)}<div style={{ margin: '14px 0', display: 'flex', gap: 8, flexWrap: 'wrap' }}><span className={creative.qa?.adSpendReady ? styles.goodPill : styles.badPill}>{creative.qa?.adSpendReady ? 'AD SPEND READY' : 'NOT SPEND READY'}</span><span className={styles.warnPill}>Benchmark: {(creative.qa?.benchmarkLevel || 'unknown').replaceAll('_', ' ')}</span>{creative.qa?.visibleTextExact ? <span className={styles.goodPill}>TEXT EXACT</span> : <span className={styles.badPill}>TEXT CHECK</span>}{creative.qa?.inventedFacts ? <span className={styles.badPill}>INVENTED FACTS</span> : null}</div>{creative.qa?.amateurSignals?.length ? <div><strong style={{ fontSize: 12 }}>AMATEUR SIGNALS</strong><ul>{creative.qa.amateurSignals.map((issue) => <li key={issue} style={{ margin: '7px 0', fontSize: 12, color: '#9a3f3f' }}>{issue}</li>)}</ul></div> : null}</> : <>{metric('Creative Quality', creative.qa?.creativeQuality)}{metric('Agency Feel', creative.qa?.agencyFeel)}{metric('Scroll-stopping', creative.qa?.scrollStoppingPower)}{metric('Photo Integration', creative.qa?.photoIntegration)}{metric('Visual Sophistication', creative.qa?.visualSophistication)}{metric('CTA Prominence', creative.qa?.ctaProminence)}{metric('Mobile Legibility', creative.qa?.mobileLegibility)}{metric('Professionalism', creative.qa?.professionalism)}{metric('Footer Clearance', creative.qa?.footerClearance)}</>}<div style={{ marginTop: 16 }}><strong style={{ fontSize: 12 }}>QA ISSUES</strong>{creative.qa?.issues?.length ? <ul>{creative.qa.issues.map((issue) => <li key={issue} style={{ margin: '7px 0', fontSize: 12, color: '#66758b' }}>{issue}</li>)}</ul> : <p style={{ fontSize: 12, color: '#26744c' }}>No blocking creative issues detected.</p>}</div>{creative.qa?.status !== 'passed' ? <p style={{ padding: 12, borderRadius: 10, background: '#fff4dd', color: '#7b5310', fontSize: 12 }}><strong>Not eligible for approval.</strong> Run another {v4 ? 'V4' : v3 ? 'V3' : ''} exploration so the engine can use this benchmark feedback.</p> : null}<small className={styles.muted}>Art Director: {creative.artDirectorModel || 'legacy'} · Image: {creative.imageModel || 'legacy'} · QA: {creative.qaModel || creative.qa?.source}</small></article>
    </section>
  </div>;
}
