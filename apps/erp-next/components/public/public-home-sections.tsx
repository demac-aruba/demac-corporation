...
type IconName =
  | 'home' | 'store' | 'building' | 'vrf' | 'sales' | 'tools' | 'service' | 'diagnostic'
  | 'shield' | 'bracket' | 'electric' | 'air' | 'restaurant' | 'clinic' | 'office' | 'hotel';
...
const industries: Array<{ icon: IconName; title: string }> = [
  { icon: 'home', title: 'Homes' }, { icon: 'restaurant', title: 'Restaurants' }, { icon: 'clinic', title: 'Clinics' },
  { icon: 'office', title: 'Professional Offices' }, { icon: 'hotel', title: 'Hotels' }, { icon: 'building', title: 'Commercial Properties' },
];

const projectTypes = [
  { title: 'Residential Cooling', copy: 'Comfort-focused installation and replacement applications.', image: 'https://images.unsplash.com/photo-1761330440311-16e160cad236?auto=format&fit=crop&w=1000&q=82' },
  { title: 'Outdoor Equipment', copy: 'Condensers and mechanical equipment for residential and commercial properties.', image: 'https://images.unsplash.com/photo-1776860153678-b204dccd0f65?auto=format&fit=crop&w=1000&q=82' },
  { title: 'Commercial Systems', copy: 'Larger-capacity cooling applications for demanding operating environments.', image: 'https://images.unsplash.com/photo-1775359647433-e2d7935b8b54?auto=format&fit=crop&w=1000&q=82' },
  { title: 'Rooftop & Exterior', copy: 'Outdoor installations planned around access, airflow and the property.', image: 'https://images.unsplash.com/photo-1775508131358-6cfae3729de4?auto=format&fit=crop&w=1000&q=82' },
  { title: 'Hospitality & Properties', copy: 'Cooling applications for hotels, hospitality and larger customer properties.', image: 'https://images.unsplash.com/photo-1545065053-56b6948e260a?auto=format&fit=crop&w=1000&q=82' },
];
...
export function PublicHomeSections() {
  return <>
    ...
    <section className="approved-services" id="services">
      <div className="approved-section-title"><div><span>Our Services</span><h2>Complete cooling solutions.</h2></div><Link href="/services">View all services →</Link></div>
      <div className="approved-service-grid">
        {services.map((service) => <Link className="approved-service-card" href="/services" key={service.title}><span className="approved-service-icon"><LineIcon name={service.icon} /></span><strong>{service.title}</strong><small>{service.description}</small><b aria-hidden="true">→</b></Link>)}
      </div>
    </section>

    <section className="approved-industries" id="industries"><span className="approved-industries-label">Industries we serve</span><div className="approved-industry-list">{industries.map((industry) => <span key={industry.title}><i><LineIcon name={industry.icon} /></i>{industry.title}</span>)}</div></section>
    ...
    <section className="approved-projects" id="projects">
      <div className="approved-section-title"><div><span>Our Work</span><h2>Cooling applications & project gallery</h2></div><Link href="/project-gallery">View gallery →</Link></div>
      <div className="approved-project-grid">{projectTypes.map((project) => <Link className="approved-project-card" href="/project-gallery" key={project.title}><span className="approved-project-image" style={{ backgroundImage: `url(${project.image})` }} aria-hidden="true" /><span className="approved-project-copy"><small>PROJECT CATEGORY</small><strong>{project.title}</strong><p>{project.copy}</p></span></Link>)}</div>
    </section>

    <section className="approved-cta" id="contact"><div className="approved-cta-copy"><span>Ready for reliable cooling comfort?</span><h2>Let DEMAC help you plan the right next step.</h2><p>From routine service to a new installation or commercial project, tell us what you need and we’ll guide the request.</p></div><div className="approved-cta-actions"><Link className="public-button public-button-whatsapp approved-main-button" href="/contact?channel=whatsapp">◉ WhatsApp Us</Link><Link className="public-button public-button-primary approved-main-button" href="/contact?request=estimate">▣ Request Estimate</Link></div></section>
  </>;
}
