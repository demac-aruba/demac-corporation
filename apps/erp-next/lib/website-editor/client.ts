...
export class PublicationRecoveryRequired extends Error {
  constructor(message: string, readonly requestId: string, readonly expectedRevision: number) {
    super(message); this.name = 'PublicationRecoveryRequired';
  }
}
...
export function createEditorialRepository(actorId: string): EditorialRepository {
  ...
  let pendingRequest: { requestId: string; expectedRevision: number } | undefined;
  const writable = () => {
    if (pendingRequest) throw new PublicationRecoveryRequired('Recover or reload the pending publication before changing this draft.', pendingRequest.requestId, pendingRequest.expectedRevision);
  };
  ...
  async function call<T>(action: string, payload: Record<string, unknown> = {}): Promise<T> {
    ...
    if (!response.ok || !result?.ok) throw new Error(result?.message || 'The publishing response could not be verified. Reload or retry to check the result.');
    return result.result as T;
  }
  return {
    mode: review ? 'review' : 'live',
    async load(seed) {
      if (!review) { const result = await call<EditorSnapshot>('load'); pendingRequest = result.pendingPublicationId ? { requestId: result.pendingPublicationId, expectedRevision: result.revision } : undefined; return result; }
      await owner(actorId);
      if (!snapshot) { snapshot = { content: copy(seed), revision: 0, savedAt: '' }; published = copy(seed); }
      return copy(snapshot);
    },
    async save(changes, expectedRevision) {
      if (!review) { writable(); return call('save', { changes, expectedRevision }); }
      await owner(actorId);
    ...
    async publish(expectedRevision, requestId) {
      if (!review) {
        // Freeze the complete command, not just its ID. A lost HTTP response may
        // follow a committed write. Only load/recovery can resolve that state.
        pendingRequest ||= { requestId, expectedRevision };
        const attempt = pendingRequest;
        try {
          const result = await call<EditorSnapshot>('publish', attempt);
          pendingRequest = undefined; return result;
        } catch (cause) {
          throw new PublicationRecoveryRequired(cause instanceof Error ? cause.message : 'Publication could not be verified.', attempt.requestId, attempt.expectedRevision);
        }
      }
      await owner(actorId);
    ...
    async restore(id, expectedRevision) {
      if (!review) { writable(); return call('restore', { revisionId: id, expectedRevision }); }
    ...
    async reset(expectedRevision) {
      if (!review) { writable(); const result = await call<EditorSnapshot>('reset', { expectedRevision }); pendingRequest = undefined; return result; }
      await owner(actorId); const current = ready(expectedRevision);
    ...
    dispose() { urls.forEach((url) => URL.revokeObjectURL(url)); urls.clear(); snapshot = null; published = null; pendingRequest = undefined; revisions.length = 0; requests.clear(); },
  };
}
