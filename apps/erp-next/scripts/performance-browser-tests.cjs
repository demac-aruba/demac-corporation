...
for(const key of ['FIRESTORE_EMULATOR_HOST','FIREBASE_AUTH_EMULATOR_HOST'])if(!/^(127\.0\.0\.1|localhost):\d+$/.test(process.env[key]||''))throw Error(`Refusing non-local ${key}`);
...
const app=initializeApp({projectId:PROJECT},'performance-browser');const db=getFirestore(app);
...
const OUT=path.resolve(__dirname,'../out');const ART=path.resolve(ROOT,'performance-test-artifacts');fs.mkdirSync(ART,{recursive:true});
const server=http.createServer((req,res)=>{
  const pathname=decodeURIComponent(new URL(req.url,'http://local').pathname);let file=path.resolve(OUT,`.${pathname}`);
...
async function main(){
  ...
  const signup=await fetch(`http://${process.env.FIREBASE_AUTH_EMULATOR_HOST}/identitytoolkit.googleapis.com/v1/accounts:signUp?key=demo`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:`browser-${Date.now()}@example.test`,password:'isolated-browser-test-123',returnSecureToken:true})});
  const auth=await signup.json();assert.ok(auth.idToken);await db.collection('users').doc(auth.localId).set({role:'admin',active:true,name:'Telemetry QA Administrator'});
  let failTelemetry=false;let ingests=0;let renewals=0;let controls=0;
  for(const [name,engine] of [['chromium',chromium],['webkit',webkit]]){
    ...
    try{
      ...
      // All external HTTP is intercepted. These tests never forward a production request.
      await context.route('**/*',async(route)=>{
        ...
        if(url.pathname==='/performanceTelemetry'){
          ...
          const payload=request.postDataJSON();if(payload.action==='ingest')ingests++;if(payload.action==='set_collection')controls++;
      ...
      await page.waitForTimeout(3500);const beforeRequests=ingests;
      await page.evaluate(async()=>{
        for(let index=0;index<25;index++)await fetch('https://us-central1-demo-demac-health.cloudfunctions.net/officeBookingAuthority',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'check_availability',data:{supportSlotSelections:['private-slot-id'],technicianInstructions:'PRIVATE-INSTRUCTIONS-MUST-NOT-BE-STORED'}})});
      });
      assert.equal(ingests,beforeRequests,'No ingest per observed support request');
      await page.evaluate(()=>document.dispatchEvent(new Event('visibilitychange')));await page.waitForTimeout(2500);
      await page.getByRole('link',{name:/Performance & Health/}).click();
      await page.getByRole('heading',{name:'Performance & Health Center',exact:true}).waitFor();await page.getByRole('button',{name:'Refresh',exact:true}).waitFor();
      await page.getByRole('button',{name:'Refresh',exact:true}).click();await page.waitForTimeout(1500);
      ...
      assert.ok(response.body.metrics.some((m)=>m.name==='support_slot_validation'&&m.module==='scheduling'&&m.count>=25),'Real browser measurements for this build must reach Firestore');
      ...
      await page.screenshot({path:path.join(ART,`${name}-mobile.png`),fullPage:true});await page.setViewportSize({width:1600,height:1050});
      const raw=await page.evaluate(()=>{const key='demac.erp-next.firebase.session.v1';const value=JSON.parse(sessionStorage.getItem(key));value.expiresAt=Date.now()+60000;const text=JSON.stringify(value);sessionStorage.setItem(key,text);return text;});
      const renewBefore=renewals;failTelemetry=true;
      await page.getByRole('button',{name:'Refresh',exact:true}).click();await page.waitForTimeout(1000);
      assert.match(await page.getByTestId('health-status').textContent(),/unavailable/i);
      assert.equal(await page.evaluate(()=>sessionStorage.getItem('demac.erp-next.firebase.session.v1')),raw);assert.equal(renewals,renewBefore);
      failTelemetry=false;await page.getByRole('button',{name:'Refresh',exact:true}).click();await page.waitForTimeout(1000);
      page.on('dialog',(dialog)=>dialog.accept());
      await page.getByRole('button',{name:'Pause collection',exact:true}).click();await page.waitForTimeout(1000);
      assert.ok(controls>0);assert.match(await page.getByTestId('health-status').textContent(),/paused/i);
      await page.getByRole('button',{name:'Resume collection',exact:true}).click();await page.waitForTimeout(1000);
