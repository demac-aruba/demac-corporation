...
(async () => {
  for (const test of matrix) {
    ...
    try {
      ...
      await job.waitFor();
      await page.locator('#job-search').fill('HVAC');
      await page.locator('#department-filter').selectOption('Field Operations');
      check(await page.getByRole('button', { name: /^View / }).count() === 1, 'catalogue search and department filter narrow the visible results together');
      await job.focus();
      ...
      check(await page.locator('#job-search').inputValue() === 'HVAC', 'browser Back restores the catalogue search value');
      check(await page.locator('#department-filter').inputValue() === 'Field Operations', 'browser Back restores the catalogue department filter');
      check(await page.getByRole('button', { name: /^View / }).count() === 1, 'restored filters keep the same result set');
