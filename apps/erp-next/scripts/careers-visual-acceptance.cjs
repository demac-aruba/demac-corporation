...
(async () => {
  for (const test of matrix) {
    ...
    try {
      ...
      await page.getByRole('button', { name: 'Español — Traducción', exact: true }).click();
      check(await page.getByLabel('I reviewed this Spanish translation against the current English version.', { exact: true }).isChecked(), 'explicit bilingual preview fixture begins with reviewed Spanish');
      await page.getByLabel('Departamento · Español', { exact: true }).fill('');
      await page.getByLabel('Título del puesto · Español', { exact: true }).fill('Técnico de aire acondicionado QA');
      check(await page.getByLabel('I reviewed this Spanish translation against the current English version.', { exact: true }).isDisabled(), 'incomplete Spanish cannot be marked reviewed');
      await shot('13-editor-spanish-draft');
      await page.getByRole('button', { name: 'Save draft in preview', exact: true }).click();
      await page.getByRole('button', { name: 'Edit vacancy & questions →', exact: true }).first().click();
      await page.getByRole('button', { name: 'Español — Traducción', exact: true }).click();
      check(await page.getByLabel('Título del puesto · Español', { exact: true }).inputValue() === 'Técnico de aire acondicionado QA', 'explicit preview draft save retains unfinished Spanish');
      check(await page.locator('#job-title').inputValue() === 'HVAC Technician', 'Spanish labels do not replace the English canonical title');
