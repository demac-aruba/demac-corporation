...
const operator: AuthPrincipal = {
  ...
  active: true,
  staffId: 'STAFF-OP-1',
  capabilities: roleCapabilities.office_operator,
};
...
function task(overrides: Partial<TaskRecord> = {}): TaskRecord {
  return {
    ...
    priority: 'important',
    status: 'pending',
    assigneeStaffId: 'STAFF-OP-1',
    assigneeNameSnapshot: 'Scarlett',
    assigneePhoneSnapshot: '2975600000',
    dueAt: '2026-09-12T14:00:00.000Z',
    checklist: [{ id: 'i-1', label: 'Draft report', completed: false }],
    attachments: [],
    ...
    updatedByUserId: manager.userId,
    updatedByName: manager.displayName,
    version: 1,
    ...overrides,
  };
}
...
function shouldNotDuplicateAlreadyQueuedReminder() {
  ...
  const candidates = dueReminderCandidates({
    task: record,
    now: new Date('2026-09-11T14:05:00.000Z'),
    windowMinutes: 10,
    alreadyQueuedKeys: new Set([scheduled.key]),
  });
...
console.log('Task Tracker acceptance checks passed.');
