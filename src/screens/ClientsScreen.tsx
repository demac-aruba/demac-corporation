...
import { Client, ClientLifecycleEntry, PreferredLanguage, Property, PropertyContact, PropertyContactLanguage, PropertyContactRole, PropertyLocation, PropertyType, WhatsAppLocationMessage } from '../types';
import { DEFAULT_PHONE_COUNTRY, formatStoredPhone, normalizePhone, phoneComparisonKey, templateLanguageFor } from '../utils/phone';
import { applyAddressSuggestion, locationCoordinates, mapsMeUrl, parseLocationInput, phoneDigits, suggestArubaAddresses } from '../utils/location';
...
export function ClientsScreen() {
  const {
    currentUser, clients, properties, equipment, workOrders, invoices, whatsappLocations,
  ...
  const [form, setForm] = useState({ name: '', company: '', phone: '', phoneCountry: DEFAULT_PHONE_COUNTRY as string, whatsapp: '', whatsappCountry: DEFAULT_PHONE_COUNTRY as string, email: '', preferredLanguage: 'Español' as PreferredLanguage, address: '', zone: 'Oranjestad', neighborhood: '', locationInput: '', accessInstructions: '', landmark: '' });
  ...
  const [propertyForm, setPropertyForm] = useState({ name: 'Propiedad principal', type: 'Casa' as PropertyType, address: '', zone: 'Oranjestad', neighborhood: '', notes: '', locationInput: '', accessInstructions: '', landmark: '' });
  ...
  const resetClientForm = () => {
    setForm({ name: '', company: '', phone: '', phoneCountry: DEFAULT_PHONE_COUNTRY, whatsapp: '', whatsappCountry: DEFAULT_PHONE_COUNTRY, email: '', preferredLanguage: 'Español', address: '', zone: 'Oranjestad', neighborhood: '', locationInput: '', accessInstructions: '', landmark: '' });
    setDuplicateResolution('block');
    setDuplicateReason('');
  };
...
const copyLocation = (property: Property) => {
  ...
  const createClient = async () => {
    ...
    const initialLocation = buildLocation(form.locationInput, /^https?:/i.test(form.locationInput.trim()) ? 'Enlace pegado' : 'Coordenadas');
...
const styles = StyleSheet.create({
  ...
  addressSuggestions: { gap: 6, marginTop: -5, marginBottom: 12 },
