...
export function TechnicianEquipmentProfileScreen() {
  const { clients, properties } = useAppState();
  const {
    workVisits,
    visitUnits,
    workInterventions,
  ...
  const interventions = workInterventions
  ...
  const primaryIntervention = interventions.find((item) => item.isPrimary);
  ...
  async function createIntervention(definition: WorkTypeDefinition) {
    ...
    const duplicate = interventions.some((item) => item.type === definition.type && item.status !== 'cancelled');
    if (duplicate) {
    ...
    <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
      ...
      <Card>
        ...
        <View style={styles.scopeGrid}>
          ...
          <View style={styles.scopeBox}>
            ...
            <Text style={styles.scopeMeta}>{interventions.length} intervención(es) registrada(s)</Text>
          </View>
        </View>
      </Card>
      ...
      <Card>
        <SectionTitle
          ...
          subtitle={primaryIntervention ? 'Puedes añadir otro trabajo sobre el mismo aire' : 'La primera selección será el trabajo principal'}
        />

        {interventions.map((intervention) => (
          <View key={intervention.id} style={[styles.interventionRow, intervention.isPrimary && styles.primaryIntervention]}>
            <View style={{ flex: 1 }}>
        ...
        <View style={styles.workTypeGrid}>
          {WORK_TYPES.map((definition) => {
            const alreadyAdded = interventions.some((item) => item.type === definition.type && item.status !== 'cancelled');
            return (
...
const styles = StyleSheet.create({
  ...
  interventionRow: { flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 13, marginBottom: 8 },
