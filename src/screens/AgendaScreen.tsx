...
export function AgendaScreen() {
  ...
  const createOrder = async () => {
    ...
    if (!service) return setFormMessage('Selecciona el tipo de servicio.');
  ...
  return (
    <ScrollView contentContainerStyle={styles.page}>
      ...
      <Card>
        <View style={styles.topPlanner}>
          ...
          <View style={styles.serviceSelect}>
            ...
            <View style={{ flex: 1 }}><Text style={styles.fieldCaption}>Servicio requerido</Text><Text style={styles.serviceName}>{selectedService?.name}</Text></View>
            <Text style={styles.chevron}>⌄</Text>
          </View>
          <View style={styles.durationBox}><Text style={styles.fieldCaption}>Duración total</Text><Text style={styles.durationValue}>{requiredSlots} cupo{requiredSlots > 1 ? 's' : ''}</Text></View>
        </View>
      </Card>
      ...
      <View style={[styles.layout, compact && styles.layoutCompact]}>
        ...
        <Card style={styles.boardCard}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={styles.boardGrid}>{vans.slice(0, 4).map((van) => <VanColumn key={van.id} van={van} users={users} orders={orders} services={services} clients={clients} selectedOrderId={selectedOrder?.id} onSelectOrder={setSelectedOrderId} onCreate={(slot) => openCreate(van.id, slot)} />)}</View>
          </ScrollView>
      ...
      <AppModal visible={showCreate} title="Confirmar nueva cita" onClose={() => !saving && setShowCreate(false)}>
        <ScrollView>
          <Text style={styles.modalIntro}>Selecciona cliente, propiedad, servicio, horario y van. La cita se guardará en la base de datos real.</Text>
          ...
          {clientProperties.length ? <View style={styles.optionWrap}>{clientProperties.map((property) => <Option key={property.id} label={`${property.name} · ${property.address}`} active={propertyId === property.id} onPress={() => setPropertyId(property.id)} />)}</View> : <Text style={styles.fallbackText}>Se usará la dirección principal del cliente.</Text>}
          ...
          <View style={styles.summaryBox}><Text style={styles.summaryTitle}>Resumen</Text><Text style={styles.summaryLine}>{selectedClient?.name ?? 'Sin cliente'} · {selectedProperty?.name ?? selectedClient?.address ?? 'Sin dirección'}</Text><Text style={styles.summaryLine}>{selectedService?.name} · {selectedVan?.name} · {formatDate(selectedDate)} · {time}</Text></View>
          <View style={styles.modalActions}><Button variant="secondary" label="Cancelar" disabled={saving} onPress={() => setShowCreate(false)} /><Button label={saving ? 'Guardando…' : 'Confirmar cita'} disabled={saving || !clients.length} onPress={() => void createOrder()} /></View>
        </ScrollView>
      </AppModal>
    </ScrollView>
  );
...
function VanColumn({ van, users, orders, services, clients, selectedOrderId, onSelectOrder, onCreate }: { van: Van; users: { id: string; name: string }[]; orders: WorkOrder[]; services: ServiceType[]; clients: Client[]; selectedOrderId?: string; onSelectOrder: (id: string) => void; onCreate: (slot: string) => void }) {
  ...
  return <View style={styles.vanColumn}><View style={styles.vanColumnHeader}><Text style={styles.vanIcon}>🚐</Text><Text style={styles.vanTitle}>{van.name}</Text><Text style={styles.vanTechs}>{techNames}</Text></View><SlotGroup title="MAÑANA" slots={morningSlots} van={van} orders={orders} services={services} clients={clients} selectedOrderId={selectedOrderId} onSelectOrder={onSelectOrder} onCreate={onCreate} /><SlotGroup title="TARDE" slots={afternoonSlots} van={van} orders={orders} services={services} clients={clients} selectedOrderId={selectedOrderId} onSelectOrder={onSelectOrder} onCreate={onCreate} /></View>;
}
