...
export function InventoryScreen() {
  ...
  async function uploadEvidence(
    ...
    const [evidenceResult, assetResult] = await Promise.all([
      ...
      module.saveVanAsset(updatedAsset),
    ]);
  ...
  async function uploadPhotosForAssets(
    ...
    const results = await Promise.all(tasks.map(async ({ asset, photo }) => {
      ...
      completed += 1;
      setRegistrationProgress(`Fotografías guardadas: ${completed} de ${tasks.length}`);
  ...
  async function registerTool() {
    ...
    try {
      ...
      setRegistrationProgress(`Subiendo ${created.assets.length === 1 ? 'la fotografía' : `${created.assets.length} fotografías`}…`);
      await uploadPhotosForAssets(created.assets, toolPhotos, 'initial');
      const toolLabel = created.catalog?.name ?? 'Herramienta';
      resetNewToolForm();
      setShowNewTool(false);
      setMessage(`${toolLabel} registrada correctamente en ${selectedVan?.name ?? 'la van'}.`);
    } catch (cause) {
    ...
    <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
      ...
      {view === 'menu' ? (
        <>
          ...
          {activeCheck ? <Card style={styles.noticeCard}><Text style={styles.cardTitle}>Control en progreso</Text><Button label="Continuar control" variant="success" onPress={continueActiveCheck} /></Card> : null}
        </>
      ) : null}
        ...
        <>
          ...
          <Card style={styles.actionCard}>
            <View style={styles.actionCardText}>
              <Text style={styles.cardTitle}>¿Qué deseas hacer?</Text>
              <Text style={styles.cardText}>El formulario de registro se abre solamente cuando lo necesitas. El listado de la van permanece limpio y fácil de revisar.</Text>
            </View>
            <Button icon="＋" label="Agregar nueva herramienta" onPress={openNewToolForm} />
          </Card>

          <Card>
            <SectionTitle title={`Herramientas de ${selectedVan.name}`} subtitle="Listado activo, faltantes y reemplazos de esta van." />
                  ...
                  {assets.length
                    ? assets.map((asset) => (
                      <AssetEditorCard
                        ...
                        onLifecycle={() => openLifecycle(asset)}
                      />
                    ))
      ...
      {view === 'checks-menu' ? (
        <View style={styles.menuGrid}>
          <MenuCard icon="🚐" title="Control de una van" text="Selecciona la van y confirma antes del checklist." label="Seleccionar van" onPress={() => openVanSelection('check')} />
      ...
      {view === 'check-van-ready' && selectedVan ? (
        <>
          ...
          <Card>
            ...
            <Button label={`Comenzar control de ${selectedVan.name}`} variant="success" disabled={module.busy || !selectedAssets.length} onPress={() => void startCheck('van')} />
          </Card>
        </>
      ) : null}
      ...
      {view === 'check-active' && activeCheck ? (
        <Card>
          <SectionTitle title={activeCheck.scope === 'van' ? `Control de ${module.vans.find((van) => van.id === activeCheck.vanId)?.name ?? 'van'}` : 'Conteo físico del depósito'} />
            ...
            ? completedChecks.map((check) => (
              <View key={check.id} style={styles.historyRow}>
                <View style={styles.flexOne}>
                  ...
                  <Text style={styles.cardText}>{check.completedAt ? new Date(check.completedAt).toLocaleString('es-AW') : ''}</Text>
                </View>
                {check.scope === 'van'
      ...
      >
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.modalContent}>
          <Text style={styles.cardText}>Completa este formulario y luego regresarás automáticamente al listado de la van.</Text>
          <View style={styles.modeGrid}>
            <ModeCard active={trackingMode === 'individual'} title="Control individual" text="Una foto e historial por power tool." onPress={() => setTrackingMode('individual')} />
            <ModeCard active={trackingMode === 'quantity'} title="Control por cantidad" text="Conteo para herramientas pequeñas." onPress={() => setTrackingMode('quantity')} />
          </View>
          ...
          <PhotoSlots
            title={trackingMode === 'individual' ? 'Fotografía obligatoria por unidad' : 'Fotografía general obligatoria'}
            slots={toolPhotos}
            mode={trackingMode}
          ...
          <View style={styles.modalActions}>
            ...
            <Button label={registrationBusy ? 'Guardando…' : `Registrar en ${selectedVan?.name ?? 'la van'}`} disabled={registrationBusy || module.busy} onPress={() => void registerTool()} />
          </View>
        </ScrollView>
      </AppModal>
      ...
      >
        {addCatalog ? (
          <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.modalContent}>
            ...
            />
            {registrationProgress ? <View style={styles.progressBox}><Text style={styles.progressText}>{registrationProgress}</Text></View> : null}
            <Button label={registrationBusy ? 'Guardando…' : `Asignar a ${selectedVan?.name ?? 'la van'}`} disabled={registrationBusy || module.busy} onPress={() => void addUnits()} />
          </ScrollView>
        ) : null}
      </AppModal>
    ...
    <View style={styles.assetCard}>
      ...
      {historicalEvidence.length ? (
        <View style={styles.historySection}>
          <Text style={styles.smallLabel}>FOTOS ANTERIORES</Text>
...
const styles = StyleSheet.create({
  ...
  actionCard: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 14, borderColor: colors.primary, backgroundColor: colors.primaryLight },
  actionCardText: { flex: 1, minWidth: 230, gap: 4 },
  modeGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  modeCard: { flex: 1, minWidth: 210, gap: 7 },
  ...
  photoSlot: { width: 210, gap: 7 },
  preview: { width: 210, height: 150, borderRadius: 9 },
  photoPlaceholder: { width: 210, height: 120, borderWidth: 1, borderColor: colors.border, borderRadius: 9, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFFFFF' },
  vanGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12 },
  vanCard: { minWidth: 220, flex: 1, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 14, gap: 7 },
  banner: { borderColor: colors.primary },
  bannerTop: { flexDirection: 'row', gap: 12, alignItems: 'center' },
  eyebrow: { color: colors.primary, fontWeight: '900', fontSize: 9, letterSpacing: 1 },
  bannerName: { color: colors.text, fontWeight: '900', fontSize: 22 },
  catalogGroup: { borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 14, marginTop: 14, gap: 10 },
  catalogHeader: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 9 },
  catalogText: { flex: 1, minWidth: 230 },
  catalogStats: { color: colors.primaryDark, fontWeight: '800', fontSize: 10, marginTop: 4 },
  shortageText: { color: colors.danger },
  unassignedBox: { backgroundColor: colors.background, padding: 10, borderRadius: 8 },
  assetCard: { borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 12, gap: 10, backgroundColor: '#FFFFFF' },
  assetTop: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 10 },
  assetImage: { width: 92, height: 72, borderRadius: 8 },
  assetImagePlaceholder: { width: 92, height: 72, borderRadius: 8, backgroundColor: colors.background, alignItems: 'center', justifyContent: 'center' },
  assetCode: { color: colors.primary, fontWeight: '900', fontSize: 10 },
  historySection: { gap: 6, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 8 },
  historyStrip: { gap: 8 },
  historyImage: { width: 90, height: 70, borderRadius: 7 },
  historyDate: { color: colors.muted, fontSize: 8, marginTop: 3 },
  retiredCard: { borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 12, gap: 8, marginBottom: 8, backgroundColor: colors.background },
  retiredReason: { color: colors.danger, fontWeight: '800', fontSize: 11 },
  lifecycleList: { gap: 4 },
  ...
  modalActions: { flexDirection: 'row', justifyContent: 'flex-end', flexWrap: 'wrap', gap: 9 },
  progressBox: { padding: 11, borderRadius: 8, backgroundColor: colors.primaryLight, borderWidth: 1, borderColor: colors.primary },
