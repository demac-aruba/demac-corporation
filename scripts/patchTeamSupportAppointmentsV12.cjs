...
const marker = 'TEAM_SUPPORT_APPOINTMENTS_V12';
const patchPath = path.join(__dirname, 'teamSupportAppointmentsV12.patch');
const targetFiles = [
...
if (marked.length === targetFiles.length) {
  console.log('patchTeamSupportAppointmentsV12.cjs already applied.');
  process.exit(0);
}
if (marked.length) throw new Error(`Team support appointments V12 is only partially applied: ${marked.join(', ')}`);
...
for (const { file, hunks } of parsed) {
  if (!expected.has(file)) throw new Error(`Unexpected file in team support appointments V12 patch: ${file}`);
  outputs.set(file, applyFilePatch(file, hunks));
}
if (outputs.size !== targetFiles.length) throw new Error('Team support appointments V12 patch is missing one or more target files.');
for (const [file, content] of outputs) fs.writeFileSync(file, content);

console.log('patchTeamSupportAppointmentsV12.cjs applied.');
