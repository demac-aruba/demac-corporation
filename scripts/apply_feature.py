...
types = replace_once(
    ...
    'PropertyContact lifecycle fields',
)
types = replace_once(
    types,
    "  contacts?: PropertyContact[];\n  active: boolean;\n  createdAt?: string;",
    "  contacts?: PropertyContact[];\n  active: boolean;\n  archivedAt?: string;\n  archivedById?: string;\n  archivedByName?: string;\n  archiveReason?: string;\n  createdAt?: string;",
    'Property lifecycle fields',
)
...
state = replace_once(
    ...
    'AppState client method type',
)
insert_after_update = """  const addProperty = async (property: Property): Promise<OperationResult> => {"""
delete_method = dedent("""
...
""")
state = replace_once(state, insert_after_update, delete_method + insert_after_update, 'deleteTestClient insertion')
state = replace_once(
    state,
    "    updateClient,\n    addProperty,",
    "    updateClient,\n    deleteTestClient,\n    addProperty,",
    'AppState value method',
)
...
rules = replace_once(
    ...
    dedent("""
        ...
        match /properties/{propertyId} {
    """),
    'clientDeletionLogs rules',
)
rules_path.write_text(rules, encoding='utf-8')

# ---- Hide archived clients from new appointments ------------------------
agenda_path = Path('src/screens/AgendaScreen.tsx')
agenda = agenda_path.read_text(encoding='utf-8')
agenda = agenda.replace('{clients.map((client) =>', '{clients.filter((client) => client.active !== false).map((client) =>')
agenda = agenda.replace('clients.map((client) =>', 'clients.filter((client) => client.active !== false).map((client) =>')
agenda_path.write_text(agenda, encoding='utf-8')

# ---- Clients screen ------------------------------------------------------
clients_screen = dedent(r'''\
...
''')
Path('src/screens/ClientsScreen.tsx').write_text(clients_screen, encoding='utf-8')
