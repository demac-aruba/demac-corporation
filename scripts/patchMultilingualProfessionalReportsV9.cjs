...
const marker = 'MULTILINGUAL_PROFESSIONAL_REPORTS_V9';
const patchPath = path.join(__dirname, 'multilingualProfessionalReportsV9.patch');
const targetFiles = [
  'src/features/technicianPortal/contracts.ts',
  'src/screens/OfficeReportReviewScreen.tsx',
  'src/services/reportPrint.ts',
  'src/services/technicalReportPdf.ts',
];

const marked = targetFiles.filter((file) => fs.readFileSync(file, 'utf8').includes(marker));
if (marked.length === targetFiles.length) {
  console.log('patchMultilingualProfessionalReportsV9.cjs already applied.');
  process.exit(0);
}
if (marked.length) throw new Error(`Multilingual report V9 is only partially applied: ${marked.join(', ')}`);
...
for (const { file, hunks } of parsed) {
  if (!expected.has(file)) throw new Error(`Unexpected file in multilingual report V9 patch: ${file}`);
  outputs.set(file, applyFilePatch(file, hunks));
}
if (outputs.size !== targetFiles.length) throw new Error('Multilingual report V9 patch is missing one or more target files.');
for (const [file, content] of outputs) fs.writeFileSync(file, content);

console.log('patchMultilingualProfessionalReportsV9.cjs applied.');
