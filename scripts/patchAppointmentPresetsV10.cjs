...
const marker = 'APPOINTMENT_PRESETS_V10';
const patchPath = path.join(__dirname, 'appointmentPresetsV10.patch');
const targetFiles = [
  'src/screens/AgendaScreen.tsx',
  'src/screens/TechnicianScreen.tsx',
...
if (marked.length === targetFiles.length) {
  console.log('patchAppointmentPresetsV10.cjs already applied.');
  process.exit(0);
}
if (marked.length) throw new Error(`Appointment presets V10 is only partially applied: ${marked.join(', ')}`);
...
for (const { file, hunks } of parsed) {
  if (!expected.has(file)) throw new Error(`Unexpected file in appointment presets V10 patch: ${file}`);
  outputs.set(file, applyFilePatch(file, hunks));
}
if (outputs.size !== targetFiles.length) throw new Error('Appointment presets V10 patch is missing one or more target files.');
for (const [file, content] of outputs) fs.writeFileSync(file, content);

console.log('patchAppointmentPresetsV10.cjs applied.');
