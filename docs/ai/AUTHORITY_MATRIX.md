...
| Shared Project planning and booking links | `projectRecords` through authenticated `projectAuthority`; Booking Authority atomically appends shared Project links | Super Admin, Operations and Project Manager manage; Finance reads planning | Fresh provisioned roles, canonical CRM/link validation, optimistic version, number/booking claims, explicit dry-run import, planning audit; historical corrections use cancelled booking crew/lock snapshots and append-only `projectHistoricalCorrections`. No Field/payroll/financial actuals authority. Browser records remain unpublished until explicit import. |
