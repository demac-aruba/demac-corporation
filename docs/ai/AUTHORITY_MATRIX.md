...
| Shared Project planning and booking links | `projectRecords` through authenticated `projectAuthority`; Booking Authority atomically appends shared Project links. For historical capacity adjustment, `projectAuthority` authenticates/orchestrates and delegates the mutation to its internal `bookingProjectHistoricalCapacity` service, which owns the atomic Appointment/Work Order/lock/Project transaction | Super Admin, Operations and Project Manager manage; Finance reads planning | Fresh provisioned roles, canonical CRM/link validation, optimistic version, number/booking claims, explicit dry-run import, planning audit. Historical single-Van Project slot adjustment preserves the Appointment/Work Order identity and changes only canonical capacity locks plus the linked Project assignment in one Booking Authority transaction, with request audit, local finance/Field evidence checks and explicit no-billing attestation; it never writes technician actual hours. The old cancel/replacement path remains available only to recover a booking already cancelled, with fresh Field/commercial/claim checks and no-billing attestation; the UI does not initiate cancellation for a new correction. No Field/payroll/financial actuals authority. Browser records remain unpublished until explicit import. |
