...
/** Prepare every relationship read before the caller writes parent/children atomically. */
async function preparePropertyLocationWrite({ db, transaction, property, data, identity, stamp }) {
  ...
  if (!Array.isArray(data.rows) || data.rows.length > 100) fail('invalid_batch', 'Save up to 100 locations per operation.');
  ...
  const rows = data.rows.map((input, index) => {
    ...
    if (kind === 'dwellings' && !['main_house', 'main_office', 'apartment', 'annex'].includes(dwellingType)) fail('invalid_dwelling_type', 'Choose main house, main office, apartment or annex.');
    return {
  ...
  if (kind === 'dwellings') {
    const count = combined.filter((row) => row.active !== false).length;
    Object.assign(summary, { hasIndependentDwellings: count > 0, dwellingCount: count });
  }
  return {
    ids: rows.map((row) => row.id), version: version + 1, summary, writeCount: rows.length + assignmentWrites.length,
    audit: { before: existing.filter((row) => ids.has(row.id)), after: rows, contactAssignmentsAfter: assignmentWrites },
    write() {
...
async function preparePropertyEditorWrite(options) {
  const dwellings = await preparePropertyLocationWrite(options);
  const areas = options.data.areas === undefined ? null : await preparePropertyLocationWrite({
    ...options, data: { ...options.data, kind: 'areas', rows: options.data.areas },
  });
  if (dwellings.writeCount + (areas?.writeCount || 0) > 200) fail('location_batch_too_large', 'Save fewer location changes at a time.');
  return { ...dwellings, audit: { ...dwellings.audit, ...(areas ? { areasBefore: areas.audit.before, areasAfter: areas.audit.after } : {}) },
    write() { dwellings.write(); areas?.write(); },
  };
}
...
function createPropertyLocationService({ db, now = () => new Date().toISOString() }) {
  ...
  async function save(data, identity) {
    ...
    return db.runTransaction(async (transaction) => {
      ...
      const plan = await preparePropertyLocationWrite({ db, transaction, property, data, identity, stamp: now() });
      plan.write();
      transaction.set(propertyRef, plan.summary, { merge: true });
      transaction.create(ledgerRef, { operation: `save_property_${kind}`, fingerprint, actorId: identity.uid, customerId, propertyId, ids: plan.ids, version: plan.version, occurredAt: now(), ...plan.audit });
      return { success: true, replayed: false, ids: plan.ids, version: plan.version };
    });
  }
  return { list, save };
}

module.exports = { createPropertyLocationService, preparePropertyLocationWrite, preparePropertyEditorWrite, resolvePropertyLocation, locationRef };
