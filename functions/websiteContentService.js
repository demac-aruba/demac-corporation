...
/** Public Storage JSON is publication authority. Existing Firestore documents
 * remain draft/audit records. Release receipts support recovery, never a second
 * public source. Ports permit failure/concurrency tests without live access. */
function createWebsiteContentService({ store, media, clock = () => new Date().toISOString() }) {
  ...
  function editableDraft(state, publicState) {
    const raw = state.draft || publicState.content;
    // Preserve current links/actions and structural identity even if a legacy
    // draft contains changes to fields this editor is not allowed to publish.
    const content = safeApply(publicState.content, changesOnly(publicState.content, normalizeVrf(raw)));
    return { ...content, id: PAGE.draftId, editorRevision: raw.editorRevision || 0, editorBaseGeneration: raw.editorBaseGeneration || publicState.generation, updatedAt: raw.updatedAt || '', updatedBy: raw.updatedBy || '' };
  }
  const snapshot = (draft, pendingPublicationId) => ({ content: normalizeVrf(draft), revision: draft.editorRevision || 0, savedAt: draft.updatedAt || '', ...(pendingPublicationId ? { pendingPublicationId } : {}) });
  function revision(draft, expected) {
  ...
  async function execute(actor, command) {
    ...
    if (action === 'load') {
      ...
      return snapshot(editableDraft(state, publicState), state.pending?.id);
    }
    ...
    if (['save', 'restore', 'reset'].includes(action)) {
      ...
      return store.transaction(async (state) => {
        ...
        const draft = editableDraft(state, publicState); revision(draft, command.expectedRevision);
        ...
        const next = { ...content, id: PAGE.draftId, editorRevision: draft.editorRevision + 1, editorBaseGeneration: action === 'save' ? draft.editorBaseGeneration : publicState.generation, updatedAt: clock(), updatedBy: actor.uid };
    ...
    if (!release) {
      ...
      release = await store.transaction(async (state) => {
        assertOwner(state.profile);
        if (state.release) return { result: state.release }; // Concurrent exact retry.
        ...
        if (/"imageUrl"\s*:\s*"(?:blob:|data:)/.test(JSON.stringify(state.draft || {}))) throw fail('unpublished-image', 'Upload browser-only images before publishing.');
        ...
        return { release: record, pending: { id: record.id }, result: record };
    ...
    if (!matches(publicState, release)) {
      try { await media.compareAndWrite(release.nextContent, release.expectedGeneration, release.id); }
        ...
        // The same request may have won on another worker, or the client may
        // have lost the response after Storage committed. Inspect before retry.
    ...
    return store.transaction(async (state) => {
      assertOwner(state.profile);
      if (state.release?.status === 'published') return { result: { ...snapshot(state.draft || release.nextContent), revision: release.draftRevision, publicationId: release.id } };
