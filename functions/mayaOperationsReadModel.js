...
function scopeCustomer(appointment, customer, property) {
  const customerOk = customer?.id === appointment.customerId;
  const propertyOk = property?.id === appointment.propertyId && property?.clientId === appointment.customerId;
  return {
...
function cancellationRow(appointment, customer, property) {
  ...
  return {
    ...
    workLines: (Array.isArray(appointment.workLines) ? appointment.workLines : []).slice(0, 30).map(line => ({
      service: cleanText(line.label || line.presetId || line.serviceId, 180),
      quantity: Number.isFinite(line.quantity) ? line.quantity : null,
    })),
  };
}
function waitlistRow(record, customer, property, appointment, today) {
  if (record.caseType !== 'booking_interest') return null;
  const interest = record.bookingInterest;
  if (!interest || !['new_appointment', 'earlier_appointment'].includes(interest.kind)) return null;
  ...
  if (state === 'waiting' && earlier && appointment.date < today) state = 'expired';
  return {
...
function createMayaOperationsReadModel({ db, clock = () => new Date() } = {}) {
  ...
  async function listWaitlist(data = {}) {
    ...
    const snapshots = (await query.limit(size + 1).get()).docs;
    const page = snapshots.slice(0, size);
    const today = arubaDateParts(clock()).date;
    const rows = await Promise.all(page.map(async snapshot => {
      ...
      return waitlistRow(record, customer, property, appointment, today);
    }));
    return { success: true, version: VERSION, rows: rows.filter(Boolean),
      nextCursor: snapshots.length > size ? page[page.length - 1].id : null,
...
module.exports = { CANCELLED, VERSION, cancellationRange, cancellationRow, createMayaOperationsReadModel, dateKey, documentId, failure, pageSize, waitlistRow };
