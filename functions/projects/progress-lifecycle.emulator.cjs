...
const { storageStatusFromCanonical, projectCanonicalWorkVisit } = require('../fieldOperationsAuthorityWorkVisit');
...
async function fixture({ phases = true, approved = true, pendingSupport = false } = {}) {
  ...
  for (const id of ids) {
    ...
    await db.collection('workVisits').doc(visitId).set({ id: visitId, workOrderId: id, appointmentId,
      clientId: base.customerId, propertyId: base.propertyId, status: storageStatusFromCanonical(complete ? 'completed' : 'scheduled'), version: 2,
