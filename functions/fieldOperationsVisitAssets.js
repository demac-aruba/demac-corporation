...
function projectVisitAsset(record) {
  ...
  const visitId = text(record?.visitId, 180);
  const assetId = text(record?.assetId, 180);
  if (!visitId || !assetId) {
    throw fieldError('visit_asset_identity_conflict', 'Persisted Visit Asset identity is incomplete.', 409);
  }
  return {
    ...
    source,
    status,
    addedOnSite: record?.addedOnSite === true,
    ...
    version: Number.isSafeInteger(record?.version) && record.version >= 1 ? record.version : 1,
  };
}

async function loadVisitAssets(db, visitId) {
  ...
  return async function attachExistingVisitAsset({ identity, visitId, assetId, requestId } = {}) {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (existingSnapshot.exists) {
        ...
        const projected = projectVisitAsset(existing);
        if (projected.visitId !== normalizedVisitId || projected.assetId !== normalizedAssetId) {
      ...
      const existingAssets = snapshotRecords(existingAssetsSnapshot).map(projectVisitAsset);
      ...
      result = {
        success: true,
        replayed: false,
        visitAsset: projectVisitAsset(visitAsset),
        allowedActions: context.allowedActions,
