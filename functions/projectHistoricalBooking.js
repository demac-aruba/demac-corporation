...
function normalizeCorrection(input) {
  const value = { projectId: identifier(input.projectId), sourceAppointmentId: identifier(input.sourceAppointmentId),
    ...
    noBillingAcknowledged: input.noBillingAcknowledged === true,
    overBudgetAcknowledged: input.overBudgetAcknowledged === true };
  ...
  if (!value.noBillingAcknowledged) fail('Confirm that this booking has no invoice or payment, including outside DEMAC ERP.');
  return value;
}
async function readEvidence(db, input, uid, now, transaction) {
  ...
  const linkClaimRef = db.collection('projectBookingClaims').doc(input.sourceAppointmentId);
  const [projectSnap, sourceSnap, claimSnap, linkClaimSnap] = await Promise.all([get(projectRef), get(sourceRef), get(claimRef), get(linkClaimRef)]);
  ...
  if (claimSnap.exists) fail('This cancelled booking already has a replacement. Further correction requires manual review.', 'conflict');
  const linkClaim = linkClaimSnap.exists ? linkClaimSnap.data() : null;
  if (!linkClaim || linkClaim.projectId !== input.projectId || linkClaim.appointmentId !== input.sourceAppointmentId
    || (linkClaim.phaseId && linkClaim.phaseId !== input.phaseId)
    || (source.projectId && source.projectId !== input.projectId)
    || (source.projectPhaseId && source.projectPhaseId !== input.phaseId)) fail('Published Project booking identity must be reconciled before correction.');
  ...
  const workOrderId = identifier(source.workOrderIds[0]);
  const sourceLinks = project.assignments.filter(link => link.appointmentId === input.sourceAppointmentId);
  if (sourceLinks.length !== 1 || sourceLinks[0].phaseId !== input.phaseId || sourceLinks[0].workOrderId !== workOrderId) {
    fail('Published Project booking and Work Order links disagree.');
  }
  const sourceLink = sourceLinks[0];
  if ((sourceLink.scheduledDate && sourceLink.scheduledDate !== source.date)
    || (sourceLink.scheduledStart && sourceLink.scheduledStart !== source.startTime)
    || (sourceLink.scheduledEnd && sourceLink.scheduledEnd !== source.capacityEndTime)
    || (sourceLink.vanId && sourceLink.vanId !== assignment.vanId)
    || (sourceLink.technicianIds && JSON.stringify([...sourceLink.technicianIds].sort())
      !== JSON.stringify([...assignment.technicianIds].sort()))) {
    fail('Project planning link differs from the cancelled appointment. Reconcile the prior reschedule first.');
  }
  assertNoProjectAssignmentExecution(sourceLinks[0]);
  ...
  if (!order || order.appointmentId !== input.sourceAppointmentId || order.clientId !== project.customerId || order.propertyId !== project.siteId
    ...
    || (order.projectId && order.projectId !== input.projectId)
    || (order.projectPhaseId && order.projectPhaseId !== input.phaseId)
  ...
  await assertNoFieldExecutionEvidence({ db, get, appointment: source, order, workOrderId, appointmentId: input.sourceAppointmentId, allowCancelled: true });
  await assertNoLinkedCommercialEvidence({ db, get, appointment: source, order, workOrderId, appointmentId: input.sourceAppointmentId });
...
function createHistoricalProjectProvider({ db, input: rawInput, uid }) {
  ...
  return {
    ...
    async prepareCommit({ transaction, now, appointmentId }) {
      ...
      const usage = await readProjectUsage(db, ref => transaction.get(ref), current.project);
      const usedAfter = usage.usedSlots + input.slots;
      if (usedAfter > usage.budgetSlots && !input.overBudgetAcknowledged) {
        fail('This recovery exceeds the approved Project slot budget. Explicit over-budget acknowledgement is required.');
      }
      ...
      return { fields: projectFields, write({ workOrders }) {
        ...
        transaction.set(current.projectRef, { ...current.project, assignments: [...current.project.assignments, link],
          scheduledFutureHours: usage.unpostedScheduledSlots + input.slots,
        ...
        transaction.set(current.claimRef, { ...projectFields, appointmentId, workOrderIds: workOrders.map(item => item.id), reason: input.reason,
          ...
          budgetSlots: usage.budgetSlots, usedBefore: usage.usedSlots, usedAfter,
          noBillingAcknowledged: input.noBillingAcknowledged,
          overBudgetAcknowledged: input.overBudgetAcknowledged });
      } };
    },
    buildWorkOrders,
  };
