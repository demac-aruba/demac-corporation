...
test('transition_visit authenticates and forwards optimistic concurrency inputs without trusting client authority', async () => {
  ...
  const result = await api.handle(request({ token: 'valid-token', action: 'transition_visit', data: { visitId: ' visit-WO-1 ', to: ' pending ', expectedVersion: 1, pendingReason: ' Waiting for part ', pendingAction: ' Office orders it ', requestId: ' transition-route-001 ', allowedActions: ['execute', 'price.override'] } }));
  assert.equal(result.status, 200);
  assert.equal(result.body.version, 1);
  assert.equal(calls.length, 1);
