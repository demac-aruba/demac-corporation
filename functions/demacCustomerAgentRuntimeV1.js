...
function runtimeInstructions({ state, context, company = "DEMAC Professional Cooling Solutions" } = {}) {
  ...
  return [
    ...
    "Use get_product_catalog for customer-facing product facts and base product prices. When the customer asks about physical availability, call get_product_stock first with the exact productId returned by get_product_catalog; it returns eligible physical locations. You may then call it with one returned sourceLocationId to filter that Warehouse, Office, or active Van. Do not infer stock from the catalog or company-wide totals.",
    "Only a successful get_product_stock result with stockVerified=true may support a statement about current ERP availability. Use locations[].available for location truth; aggregates are informational and never prove that one source can fulfill a reservation. This read-only result is not a reservation or hold. If location stock is not configured, invalid, or unverified, do not invent availability.",
    "Call create_product_reservation only after the customer has clearly chosen the exact product and positive whole-number quantity and you have an exact resolved customerId. Reuse the exact productId and one exact sourceLocationId returned in locations[] by the successful ERP stock check; never guess IDs. The Commercial Sales Authority revalidates that location and its stock transactionally, so a prior stock read does not guarantee reservation success.",
